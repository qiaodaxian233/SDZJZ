...
| **数据包格式**（m527） | 1.21：`data/<ns>/recipe/`、`loot_table/`，配方 `result: {id, count}`，标签目录单数 `tags/block/`；1.20.1：`recipes/`、`loot_tables/`，`result: {item, count}`，`tags/blocks/`；本世代未注册的 id 出现在配方里=加载报 "Failed to parse recipe" | **1.20.1 `data/` 不许手写**：`docs/tools_datapack_1201.py --write` 从主线机械转换（四处差+未注册 id 整条跳过并打印），第 22 闸校验漂移；主线加配方/本世代注册新件后重跑 --write | m527 |
