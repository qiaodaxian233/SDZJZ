...
public class StructureCoreScreenHandler extends AbstractContainerMenu {
    ...
    // 客户端
    public StructureCoreScreenHandler(int syncId, Container playerInv, BlockPos pos) {
        this(syncId, playerInv, resolve(playerInv, pos), new SimpleContainerData(10));
    }

    // 服务端
    public StructureCoreScreenHandler(int syncId, Container playerInv, StructureCoreBlockEntity be) {
        this(syncId, playerInv, be, be != null ? be.propertyDelegate : new SimpleContainerData(10));
    }

    private StructureCoreScreenHandler(int syncId, Container playerInv, StructureCoreBlockEntity be, ContainerData props) {
        ...
        if (be != null && playerInv.player instanceof net.minecraft.server.level.ServerPlayer sp)
            be.addCanvasViewer(sp); // m344 开屏挂号（客户端构造 player 非 ServerPlayer，天然不进）
    }
    ...
    private static StructureCoreBlockEntity resolve(Container playerInv, BlockPos pos) {
