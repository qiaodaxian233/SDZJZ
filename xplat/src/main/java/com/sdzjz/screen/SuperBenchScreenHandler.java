...
public class SuperBenchScreenHandler extends AbstractContainerMenu {
    ...
    // 客户端
    public SuperBenchScreenHandler(int syncId, Container playerInv) {
        this(syncId, playerInv, ContainerLevelAccess.EMPTY);
    }

    public SuperBenchScreenHandler(int syncId, Container playerInv, ContainerLevelAccess context) {
    ...
    @Override
    public void onContentChanged(Container inv) {
        if (inv == input) {
    ...
    private void consumeIngredients() {
        ...
        for (Map.Entry<String, Integer> e : r.ingredients().entrySet()) {
            ...
            for (int i = 0; i < GRID_SLOTS && need > 0; i++) {
                ...
                if (BuiltInRegistries.ITEM.getId(s.getItem()).toString().equals(e.getKey())) {
    ...
    public boolean onButtonClick(Player player, int id) {
        ...
        for (String mob : r.mobs()) {
            Container pinv = player.getInventory();
        ...
        for (int pass = 0; pass < 3 && need > 0; pass++) {
            ...
            for (int i = 0; i < pinv.size() && need >= ratio; i++) {
                ...
                if (pass < 2) {
                    ...
                    if (!BuiltInRegistries.ITEM.getId(s.getItem()).toString().equals(id)) continue;
                }
    ...
    private void sendMissingSummary(Player player, SuperBenchRecipes.Recipe r) {
        ...
        if (missing.isEmpty() && cageMiss.isEmpty()) {
            player.sendMessage(net.minecraft.network.chat.Component.literal("材料齐全，取走结果即可")
                    .formatted(net.minecraft.ChatFormatting.GREEN), false);
            return;
        }
        net.minecraft.network.chat.MutableComponent msg = net.minecraft.network.chat.Component.literal("还缺: ")
                .formatted(net.minecraft.ChatFormatting.RED);
        if (!cageMiss.isEmpty()) {
    ...
    private void compressGrid(Player player) {
        ...
        for (int i = 0; i < GRID_SLOTS; i++) {
            ...
            plain.merge(BuiltInRegistries.ITEM.getId(s.getItem()).toString(), s.getCount(), Integer::sum);
        }
        ...
        if (t1Made == 0 && t2Made == 0) {
            player.sendMessage(net.minecraft.network.chat.Component.literal(
                    ...
                    .formatted(net.minecraft.ChatFormatting.GRAY), false);
        } else {
            player.sendMessage(net.minecraft.network.chat.Component.literal(
    ...
    private void unpackGrid(Player player) {
        ...
        if (t2Opened + t1Opened > 0) {
            ...
            player.sendMessage(net.minecraft.network.chat.Component.literal(msg)
                    .formatted(spaceOut ? net.minecraft.ChatFormatting.YELLOW : net.minecraft.ChatFormatting.GREEN), false);
        } else if (spaceOut) {
            player.sendMessage(net.minecraft.network.chat.Component.literal("网格和背包都装不下拆出来的东西——先腾地方再点")
                    .formatted(net.minecraft.ChatFormatting.RED), false);
        } else {
            ...
            player.sendMessage(net.minecraft.network.chat.Component.literal(invHasPack
                    ? "拆开只认网格里的包——把背包里的包放进左边网格再点"
                    : "网格里没有材料包").formatted(net.minecraft.ChatFormatting.GRAY), false);
        }
    }
