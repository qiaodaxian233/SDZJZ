 ...
 *  m201 接原版工作台接口：继承 RecipeBookMenu（原版配方书 CraftRequest 协议直通），
...
public class DataPanelScreenHandler extends net.minecraft.world.inventory.RecipeBookMenu<
    ...
    public DataPanelScreenHandler(int syncId, Container playerInv, BlockPos pos) {
        this(syncId, playerInv, resolve(playerInv, pos));
    }

    public DataPanelScreenHandler(int syncId, Container playerInv, DataPanelBlockEntity be) {
        this(syncId, playerInv, be, false); // m303 缺省=方块开屏（BE.createMenu 与客户端工厂都走这，零调用点改动）
    }

    public DataPanelScreenHandler(int syncId, Container playerInv, DataPanelBlockEntity be, boolean remote) {
        ...
        // 结果格=原版 ResultSlot 子类：EMI CoercedRecipeHandler 按 instanceof 认它（零插件点亮）；
    ...
    public boolean onButtonClick(Player player, int id) {
        ...
        if (id == 1) {
            ...
            sp.setExperienceLevel(0);  // 这两个 setter 在 ServerPlayer 上（Yarn 1.21 查证），
    ...
    private static DataPanelBlockEntity resolve(Container playerInv, BlockPos pos) {
    ...
    // ===== m201 原版工作台接口实现（RecipeBookMenu；名称全按 Yarn 1.21.1 官方映射核过）=====
    @Override
     ...
     *  缺料时回发原版 ClientboundPlaceGhostRecipePacket：客户端配方书在网格画半透明 ghost 配方（原版工作台同款观感）。
     *  craftAll(shift点配方)=装满一组，单击=每格 1 个——与 jeiFill 的 max 参数语义一一对应。
     *  【待编译验证】形参按原版源写 RecipeHolder<?>（网络处理器传入的是未定型 entry）；若 CI 红改 RecipeHolder<CraftingRecipe> 重试。 */
    @Override
