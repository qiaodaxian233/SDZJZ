...
public class LinkerItem extends Item {
    ...
    public LinkerItem(Properties settings) {
        super(settings);
    }
    ...
    public InteractionResult useOn(UseOnContext ctx) {
        ...
        if (be instanceof com.sdzjz.block.DataCableBlockEntity cable) { // m227：原扳手功能（m224~m226）
            if (player != null) {
                ...
                if (player.isShiftKeyDown()) { // 潜行右键：手臂=断开该面连接；缆芯已断侧=恢复；缆芯=快速开/关
                    double lx = ctx.getClickLocation().x - pos.getX() - 0.5,
                           ly = ctx.getClickLocation().y - pos.getY() - 0.5,
                           lz = ctx.getClickLocation().z - pos.getZ() - 0.5;
                    net.minecraft.core.Direction hitDir = net.minecraft.core.Direction.getNearest(lx, ly, lz);
                    ...
                    if (mag > 0.14) { // m233 缆芯半宽 2/16=0.125，超过即点在手臂/插头上：断开该面
                        ...
                        player.displayClientMessage(Component.translatable(cable.faceDisabled(hitDir)
                                ? "sdzjz.extract_port.face_off" : "sdzjz.extract_port.face_on", hitDir.getName()), true);
                    } else if (cable.faceDisabled(ctx.getClickedFace())) { // m233 点缆芯的已断开侧=恢复该面
                        cable.toggleFace(ctx.getClickedFace());
                        com.sdzjz.block.DataCableBlock.refreshEnd(world, pos, ctx.getClickedFace());
                        player.displayClientMessage(Component.translatable("sdzjz.extract_port.face_on", ctx.getClickedFace().getName()), true);
                        ...
                        player.displayClientMessage(cable.extractOn()
