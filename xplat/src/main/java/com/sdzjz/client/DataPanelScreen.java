...
public class DataPanelScreen extends AbstractContainerScreen<DataPanelScreenHandler>
    ...
    protected void init() {
        ...
        this.search = new EditBox(this.font, this.leftPos + 16, this.topPos + 30, 176, 12, Component.literal("搜索"));
        this.search.setBordered(false);
        this.search.setTextColor(SciSkin.termInk()); // m200 浅面上写墨字
        // m216 撤原版 setPlaceholder：1.21.1 的 placeholder 聚焦即隐（作者实机截图=聚焦紫描边+空框无字），
        // 改 drawBackground 自绘提示——空文本时两态（聚焦/失焦）都可见，光标压在提示首字上属正常观感。
        this.search.setResponder(s -> { scroll = 0; sendView(); });
        ...
        for (int i = 0; i < 7; i++) {
            final int fi = i;
            String kv = old != null && old[i] != null ? old[i].getValue() : themeGet(c, i);
            themeF[i] = new EditBox(this.font, 0, 0, 54, 12, Component.empty());
            themeF[i].setBordered(false);
            themeF[i].setMaxLength(7);
            themeF[i].setValue(kv == null ? "" : kv);
            themeF[i].setResponder(t -> themeSet(com.sdzjz.config.SdzjzConfig.get(), fi, t.trim()));
        }
    }
    ...
    private static java.util.List<String> matchByLocalName(String q) {
        ...
        String probe = new net.minecraft.world.item.ItemStack(net.minecraft.world.item.Items.STONE).getHoverName().getString();
        if (nameIndex == null || !probe.equals(nameProbe)) {
            ...
            for (net.minecraft.world.item.Item item : net.minecraft.core.registries.BuiltInRegistries.ITEM) {
                ...
                String nm = new net.minecraft.world.item.ItemStack(item).getHoverName().getString();
    ...
    protected void renderBg(GuiGraphics ctx, float delta, int mouseX, int mouseY) {
        ...
        ctx.drawString(this.font, "存储终端", x + 28, y + 7, SciSkin.termInk(), false);
        // m97/m98 全网类型用量（满=红：核心类型上限到顶新种类被拒收的原因亮出来）
        int tu = this.menu.typesUsedView(), tc = this.menu.typesCapView();
        ...
        if (search != null && search.getValue().isEmpty()) // m216 自绘默认提示：坐标=控件文字原位(x+16,y+30)，subdued 色不与输入争眼
            ctx.drawString(this.font, "搜索物品，中文/首字母…", x + 16, y + 30, SciSkin.termSub(), false);
        ...
        int rowsAll = Math.max(6, this.menu.rowsView());
        int th = Math.max(12, 108 * 6 / rowsAll);
        int mr = Math.max(0, this.menu.rowsView() - 6);
        ...
        ctx.drawString(this.font, "背包", x + 16, y + 170, SciSkin.termSub(), false);
        for (int r = 0; r < 3; r++)
        ...
        ctx.drawString(this.font, "经验库", x + 217, y + 29, SciSkin.termSub(), false);
        ctx.drawString(this.font, fmt(this.menu.xpBankView()) + " 点", x + 217, y + 41, SciSkin.termAccentDeep(), false);
        SciSkin.termBtn(ctx, this.font, x + 211, y + 56, 66, 16, "存入经验",
                mouseX >= x + 211 && mouseX <= x + 277 && mouseY >= y + 56 && mouseY <= y + 72, false);
        SciSkin.termBtn(ctx, this.font, x + 281, y + 56, 66, 16, "取出经验",
        ...
        ctx.drawString(this.font, "合成终端", x + 217, y + 85, SciSkin.termSub(), false);
        for (int r = 0; r < 3; r++)
            for (int c = 0; c < 3; c++) SciSkin.termSlot(ctx, x + 213 + c * 18, y + 96 + r * 18);
        ctx.drawString(this.font, "▶", x + 283, y + 118, SciSkin.termAccent(), false); // 网格→结果
        ctx.fill(x + 306, y + 111, x + 328, y + 133, SciSkin.termAccentDeep()); // 结果格紫外环
        SciSkin.termSlot(ctx, x + 309, y + 114);
        SciSkin.termBtn(ctx, this.font, x + 211, y + 156, 135, 16, "清空回仓",
        ...
        ctx.drawString(this.font, "回收", x + 217, y + 187, SciSkin.termSub(), false);
    ...
    @Override
    protected void renderSlot(GuiGraphics ctx, net.minecraft.world.inventory.Slot slot) {
    ...
    public boolean mouseClicked(double mx, double my, int button) {
        ...
        if (bookOn) {
            if (button == 0 && mx >= this.leftPos + BOOK_BTN_DX && mx < this.leftPos + BOOK_BTN_DX + BOOK_BTN_W
                    && my >= this.topPos + BOOK_BTN_DY && my < this.topPos + BOOK_BTN_DY + BOOK_BTN_H) {
            ...
            if (this.narrow && this.recipeBook.isVisible()) return true;
        }
        // m107b：滚动条命中——点轨道跳页并开始拖拽（浮层打开时不抢，见上）
        int sbx = this.leftPos + 181;
        if (button == 0 && mx >= sbx - 1 && mx <= sbx + 7 && my >= this.topPos + 52 && my <= this.topPos + 160) {
            sbDrag = true;
            sbUpdate(my);
            return true;
        }
        // m111 AE 手感：光标拿着东西点存储区 = 存入（左键全放/右键放1）——原版此处是"无操作"，必须拦截
        boolean overGridClick = mx >= this.leftPos + 16 && mx < this.leftPos + 178 && my >= this.topPos + 52 && my < this.topPos + 160;
        if (overGridClick && !this.menu.getCarried().isEmpty() && (button == 0 || button == 1)) {
        ...
        if (button == 0) { // 固定钮：经验库(1存/2取)、清空回仓(3)、主题、关闭（区域与 drawBackground 同一套）
            double rx = mx - this.leftPos, ry = my - this.topPos;
            ...
            if (rx >= 336 && rx <= 352 && ry >= 4 && ry <= 20) { this.onClose(); return true; }
        }
    ...
    public boolean mouseScrolled(double mx, double my, double h, double v) {
        ...
        boolean overGrid = mx >= this.leftPos + 16 && mx <= this.leftPos + 187
                && my >= this.topPos + 52 && my <= this.topPos + 160;
        if (!overGrid) return super.mouseScrolled(mx, my, h, v);
        int mr = Math.max(0, this.menu.rowsView() - 6); // 真实 clamp，撤 bottomFull 启发式
    ...
    private void sbUpdate(double my) {
        ...
        int rowsAll = Math.max(6, this.menu.rowsView());
        int th = Math.max(12, 108 * 6 / rowsAll);
        double rel = (my - (this.topPos + 52) - th / 2.0) / (double) (108 - th);
    ...
    protected void containerTick() {
        super.containerTick();
        if (bookOn) this.recipeBook.tick(); // 背包变动→"可合成"筛选/ghost 跟着刷
    }

    @Override
    protected void slotClicked(net.minecraft.world.inventory.Slot slot, int slotId, int button,
    ...
    protected boolean hasClickedOutside(double mx, double my, int left, int top, int button) {
        boolean out = mx < left || my < top || mx >= left + this.imageWidth || my >= top + this.imageHeight;
        if (!bookOn) return out;
        // 书区不算"窗外"——否则拿着东西点书=误丢地上（原版同款合取）
        return this.recipeBook.hasClickedOutside(mx, my, this.leftPos, this.topPos,
                this.imageWidth, this.imageHeight, button) && out;
    }
    ...
    @Override
    public void recipesUpdated() { this.recipeBook.recipesUpdated(); } // 数据包重载(/reload)后原版回调
    ...
    public void onStockSync() {
        if (bookOn) this.recipeBook.recipesUpdated();
    }
    ...
    private void openTheme() {
        com.sdzjz.config.SdzjzConfig c = com.sdzjz.config.SdzjzConfig.get();
        for (int i = 0; i < 7; i++) themeF[i].setValue(themeGet(c, i)); // 开窗对齐配置现值
        themeOpen = true;
    }
    ...
    private void renderTheme(GuiGraphics ctx, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            ctx.drawString(this.font, THEME_LABELS[i], px + 8, ry + 3,
            ...
            themeF[i].setTextColor(SciSkin.termInk()); // 浅井写墨字，随主题联动
            themeF[i].setX(px + 65);
            themeF[i].setY(ry + 2);
            ...
            if (!hexOk(themeF[i].getValue())) ctx.fill(px + 62, ry + 14, px + 120, ry + 15, SciSkin.RED);
        }
        // ===== 右列：所选色 RGB 滑块 =====
        ctx.drawString(this.font, "调节: " + THEME_LABELS[thSel], px + TH_RX, py + 22, SciSkin.termInk(), false);
        int c = themeColor(thSel);
        String[] chn = {"R", "G", "B"};
        for (int ch = 0; ch < 3; ch++) {
            int v = (c >> (16 - ch * 8)) & 0xFF;
            int sy = py + 36 + ch * 18;
            ctx.drawString(this.font, chn[ch], px + TH_RX, sy + 1, SciSkin.termSub(), false);
            ...
            ctx.drawString(this.font, vs, px + SL_X + SL_W + 6, sy + 1, SciSkin.termInk(), false);
        }
        // ===== 右列：预设 =====
        int hovP = hoveredPreset(mouseX, mouseY);
        ctx.drawString(this.font, "预设" + (hovP >= 0 ? ": " + SciSkin.TERM_PRESET_NAMES[hovP] : ""),
    ...
    private void thApplySlider(double mx) {
        ...
        themeF[thSel].setValue(String.format("%02X%02X%02X", r, g, b));
    }
    ...
    private boolean themeClick(double mx, double my, int button) {
        ...
        if (pk >= 0) {
            for (int i = 0; i < 7; i++) themeF[i].setValue(SciSkin.TERM_PRESETS[pk][i]);
            com.sdzjz.config.SdzjzConfig.save();
            return true;
        }
        if (mx >= px + TH_RX && mx <= px + TH_RX + 120 && my >= py + 130 && my <= py + 146) { // 恢复默认
            com.sdzjz.config.SdzjzConfig d = new com.sdzjz.config.SdzjzConfig();
            for (int i = 0; i < 7; i++) themeF[i].setValue(themeGet(d, i)); // setText 触发 listener 回写配置
            com.sdzjz.config.SdzjzConfig.save();
        }
        return true;
    }
    ...
    public void render(GuiGraphics ctx, int mouseX, int mouseY, float delta) {
        ...
        if (bookOn && this.recipeBook.isVisible() && this.narrow) {
        ...
        if (bookOn) { // m286 主题书钮（书之后画=窄屏开书浮在书上可关；开=主紫态）
            int bbx = this.leftPos + BOOK_BTN_DX, bby = this.topPos + BOOK_BTN_DY;
            SciSkin.termBtn(ctx, this.font, bbx, bby, BOOK_BTN_W, BOOK_BTN_H, "配方",
                    mouseX >= bbx && mouseX < bbx + BOOK_BTN_W && mouseY >= bby && mouseY < bby + BOOK_BTN_H,
                    this.recipeBook.isVisible());
        ...
        if (qtySlot >= 0) { // m82 数量选择浮层 + m100 批量行（m200 换终端主题皮；m202 抬 z=400 防槽物品穿透）
            ...
            ctx.drawString(this.font, "取出数量:", qtyX, qtyY - 12, SciSkin.termHi(), false);
            for (int k = 0; k < QTY.length; k++) { // 第一行：定量
                int bx = qtyX + k * 26, by = qtyY;
                boolean hov = mouseX >= bx && mouseX <= bx + 24 && mouseY >= by && mouseY <= by + 16;
                SciSkin.termBtn(ctx, this.font, bx, by, 24, 16, String.valueOf(QTY[k]), hov, hov);
            }
