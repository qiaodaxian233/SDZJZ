...
## 模型接入铁律（用户明确要求，m58 起生效）
- **用户提供的所有 .bbmodel 都自带动画**。接入任何模型必须：①解析 `animations` 字段 ②静态件进模型 JSON、动画件拆出 ③BER 复刻关键帧（周期/通道/幅度按数据 1:1 或注明适配）④DEVLOG 记录动画实现方式。只转静态几何 = 未完成。

- **bbmodel 多贴图铁律**：转换前必看 textures[] 的 name/uv_width/uv_height——MER/光影图常排在[0]，无脑取首张=糊图；UV 制式按每张贴图自带分辨率换算，不按工程 resolution。
