...
public class SdzjzConfig {
    ...
    // ===== m460 漏斗对接 =====
    public boolean hopperDockingEnabled = true; // m460 存储核心幻影槽（原版漏斗/漏斗矿车/一切走 Container 接口的管道可往核心塞料，插入即入账、禁抽取）。关=漏斗视核心为零槽位容器（塞不进也抽不走）；FTA 直连（m161c）不受此闸影响

    // ===== m461 反向直连 =====
    public boolean xferPushEnabled = true; // m461 生产核心出货补 FTA 分支：无自家仓/箱子时，产出可直塞相邻或数据线连到的 FTA-only 机器（MI/TechReborn/Create 置物台等只暴露 fabric-transfer-api 的方块）。带组件产出走精确变体组件保真。关=出货路由回旧行为（FTA 目标视而不见）
    ...
    public boolean chunkFxEnabled = true;      // m384 区块机器特效总开关（三件一闸）：选区框（m393 起双源=手持瞄准态+核心里工作态，客户端本地读）+锁定爆发粒子环/信标音+前沿激光束（m393 换装：削切点上方 END_ROD 光柱+火花+烟羽，每台每拍封顶三束）
    public boolean infiniteBeaconEnabled = true;   // m399 无限距离信标总闸（服主政策级：本机给全服玩家常驻 buff）。关=该节点黄灯停发
    public int infiniteBeaconFuelPerCycle = 1;     // m399 每周期扣几份信标料（铁锭/金锭/绿宝石/钻石/下界合金锭，从便宜到贵依次扣）
    public int infiniteBeaconLevel2Cost = 4;       // m399 II 级每周期料倍数（1=II 级不加价）
    public int infiniteBeaconEffectSeconds = 12;   // m399 每次施加的效果时长（秒）；周期 80 拍=4 秒，留足续期余量。效果是刷新式不叠加
    public boolean infiniteBeaconCrossDimension = true; // m399 跨维度覆盖（true=全服在线玩家不论在哪层都吃到；false=只管核心所在维度）
    public int chunkRemoverMaxBlocksPerTick = 16384; // m398 每台每拍方块硬顶（原写死 4096；m395 快写后单块成本大降故默认抬 4 倍，下面还有时间片兜 tick）。升级算出的量超过它=副行标"满载(每拍方块硬顶)"，想更快就抬这个键
    public int chunkRemoverTimeSliceMs = 6;     // m398 自适应时间预算：每台每拍最多花多少毫秒在挖掘上（0=关，纯按方块硬顶）。到点即停、游标落盘下拍无缝续——这才是"加速升级拉满还慢"的正解：让真实机器性能说话，而不是拍脑袋的方块数
    public int chunkRemoverTimePoolMs = 20;     // m398 全服所有移除器每拍共享的墙钟池（0=关=每台各领满时间片）。池见底仍给每台 1ms 保底（绝不整拍不干活，m99 防哑死）；服主按自己 MSPT 余量调
    public boolean chunkRemoverVoidMode = true; // m397 空置域模式（第三挡）总闸：开=移除器可切"空置域·破基岩"，连基岩/屏障这类硬度<0 的方块一起拆（坑底会通虚空，掉下去=摔没）。关=只剩两挡，已切到空置域的机器黄灯提醒并按无掉落·极速挖
    public boolean chunkRemoverSealCustom = true; // m396 允许自定义封边材料（画布节点菜单选，自定义料从存储扣，不够=黄灯提醒并回落免费石墙）。关=全服一律按默认免费石墙，已选的材料留着不生效
    public boolean chunkRemoverFastWrite = true; // m395 Bulk Engine 第一刀：移除器世界写入用 NOTIFY_LISTENERS|FORCE_STATE（跳邻居方块更新与邻居形状更新——整片全拆时这些更新全是白干，邻居下一拍也要被拆）。副作用：拆墙不再让水/岩浆当场流进来、悬空沙砾不当场掉、红石/观察者不响应（都会在游标走到时被拆掉）。关=回 NOTIFY_ALL 逐位同旧（对表/回滚用）
    public boolean chunkHighlightRunning = true; // m393 核心里的移除器也画选区框（作者点名"放进核心也显示边框"）：客户端每 500ms 扫周边已加载分块的核心方块实体，收其画布里已绑定未清完的移除器节点。关=只有手持时才画
    public boolean chunkHighlightXray = true;   // m393 选区框穿墙透视线（原版 F3+G 区块边界同款层）：站坑里/地下也看得见圈了多大一片。关=只剩会被地形挡住的实心线框
    public int chunkHighlightScanChunks = 8;    // m393 工作态扫描半径（分块，钳 1..16）：周边多远内的核心会被收进画框名单，越大越费一点客户端遍历
    ...
    public static void load() {
        ...
        if (cfg.configVersion < 54) { // m386 区域上限默认 2→64：旧档存的是旧默认 2 则主动抬（服主手改过别的值不动），否则"无限调整"对老用户不生效
            if (cfg.chunkRemoverMaxRadius == 2) cfg.chunkRemoverMaxRadius = 64;
            cfg.configVersion = 54;
        }
        if (cfg.configVersion < 55) cfg.configVersion = 55; // m388 纯加键（chunkRemoverSealFluids），缺键走字段初值
        if (cfg.configVersion < 56) cfg.configVersion = 56; // m393 纯加键（chunkHighlightRunning/Xray/ScanChunks 选区高亮三键），缺键走字段初值
        if (cfg.configVersion < 57) cfg.configVersion = 57; // m395 纯加键（chunkRemoverFastWrite 移除器快写标志位），缺键走字段初值
        if (cfg.configVersion < 58) cfg.configVersion = 58; // m396 纯加键（chunkRemoverSealCustom 自定义封边材料闸），缺键走字段初值
        if (cfg.configVersion < 59) cfg.configVersion = 59; // m397 纯加键（chunkRemoverVoidMode 空置域模式总闸），缺键走字段初值
        if (cfg.configVersion < 60) cfg.configVersion = 60; // m398 纯加键（移除器自适应时间预算三键），缺键走字段初值
        if (cfg.configVersion < 61) cfg.configVersion = 61; // m399 纯加键（无限距离信标五键），缺键走字段初值
