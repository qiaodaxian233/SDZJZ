...
- 验证脚本：①合成需要装村民的抓捕笼；②画布插机→徽章"选交易"→列表选"图书管理员：
  纸×24→绿宝石"→仓里备纸→绿宝石自动入库；③连线：刷线机→交易机（选"制箭人"？无——选
  渔夫煤或图管纸线）用线喂料…实际验证用 刷线机出线连交易机+选图管"纸"不对——**用户场景**：
  刷线机产线→交易机目标选"渔夫：煤"不吃线——正确演示：刷线机出线→交易机，目标选制图师/
  图管的纸交易不匹配线；应选**没有线收购交易**…本表 v1 无线收购条目，DEVLOG 留痕：
  如需"线→绿宝石"（原版渔夫/制箭师有），给 VillagerTrades 补一条 t("minecraft:string",20,
  "minecraft:emerald",1) 即可，一行事；④附魔书交易：绿宝石+书自动扣，附魔书进精确账本
