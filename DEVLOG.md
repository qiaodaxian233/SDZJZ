...
- **作者点名**：「你能不能设计一个作业表啊。不要每次都让我让你怎么做。」——合理，而且指出了
  一个我一直没解决的问题：**我每刀结束都在等作者指方向**，本该自己有张表。
- **`docs/作业表.md`** 六节：
  ①**取活规则**（修构建失败 > 作者当轮点名 > 当前分片收尾 > 无阻塞主线活 > 需拍板的），
  外加一张「**我自己能决定 / 必须问作者**」的对照——技术实现、加闸改闸、重构命名我自己定；
  玩法语义、跨代行为取舍、高风险主线手术时机必须问；
  ②**每刀标准流程**（把 m483~m492 全部教训固化成 10 条 checklist：搬前先查世代 API 对照表、
  整段搬不重写、**连调用点一起搬**、切段别按行号盲切、注释里的刀号一起搬；验证用筛选器不手写
  grep、20 闸一条命令、版本与三份文档同刀更新、汇报时说清实机预期与是否动了存档路径）；
  ③下一刀；④**待办四条线**（A 画布视觉 / B 真移植剩余 / C 功能补齐 / D 欠验与债务），
  每项带依赖与验收标准；⑤**阻塞区**（等作者拍板的，我不猜）；⑥已完成归档。
- **建完立刻按表取活**，做了 A3：**节点悬停详情**。主线 m85 段整段搬成 `NodeTooltip`——
  机器名×数量、状态（核心停机优先）、**周期与基础产量**（`baseIntervalTicks`/20 秒 +
  按产出表期望值×数量算/分）、**产出表前三项**（超三项省略号）、消耗输入提示。
  照流程第 1 条先查了对照表，命中两处世代差：`instanceof MachineItem` → `defOf`（m472 口）、
  `ResourceLocation.parse` → `Gfx.id`（m484 口）——**这次是查表，不是猜**。
  1.20.1 原来只有「机器名 + 一行原因」，现在与主线同形，并保留本世代特有的阻塞原因整句。
- **A 线主体收尾**：卡面工艺（m483）→ 节点卡（m484）→ 连线缎带（m488/m492）→ 面板屏（m489）→
  小地图（m490）→ 悬停详情（m493）。剩下 A4~A8 是增强项（归并徽章/分组框选/缩放动效/设置面板/
  升级槽），**按取活规则第 4 条，C1（⑤d2 熔炉族）的玩家可见价值更高，下一刀走 C1**。
- **验证**：两代冒烟经筛选器复查均无必须看的真错；20 闸全绿（0.1.493 对表）。零配置零资源改动，
  **不碰存档与运行时路径**（纯 UI）。
- **教训**：**"等人给活"本身就是一种成本**。前面十几刀我每次收尾都把决策抛回给作者，
  而其中绝大多数（先搬哪件、怎么分片、要不要加闸）**本来就该我自己定**——作者真正需要介入的
  只有两类：拍板语义取舍、反馈实机结果。**把这条边界写下来（作业表第一节那张表），
  比每次临时判断可靠。**

## m494 C2-⑤d2：熔炉族 tick 落地——万能熔炉在 1.20.1 可玩

- **按作业表取活**（第一次完全自主取活，没等作者指方向）：A 线主体已收尾、C 线除本项外全被
  「附魔定价 A/B」阻塞、B2 需先办前置——所以取 **C1（⑤d2 熔炉族）**：无阻塞、玩家可见价值最高。
- **搬法照流程**：主线 `StructureCoreBlockEntity` 熔炉族分支（734~789，56 行）整段搬进本世代
  tick，两条料路逐句照抄——**有入线吃自己的在途缓存 + 遗留池**（m350 转存 scratch 防虚账）、
  **无入线吃 kind1 定向供料线**。`capacity` 算式、m149 白名单闸、`SmeltPlanner.resultOf` 判定、
  「无可烧材料」红灯词条全部原样。
- **本世代必须显式接线才取料**（主线原注照搬）：不做全局网络兜底——**防止把玩家存着的
  原木/圆石/粗矿悄悄全烧了**。这条是主线刻意的设计，不是限制。
- **三处世代取舍显式记档**：①无并发/数量升级（m464），`running`/`countLv` 恒 1/0，
  capacity 只保留「一组×周期数×熔炉族倍数（工程款 ×108）」；②无输出缓存/断网喷射，
  去处满=原样回灌自己缓存 + 黄灯持料（m473 同律）；③**主线此处 `xpPool += 0.1*done`
  （熔炼经验入核心池），本世代 xpPool 未建，随 ⑤e 到序**——m476 普查早就标了 ⑤d/⑤e 这处交叉。
- **顺手拆掉一颗会崩服的雷**：熔炉族每拍都问 `SmeltPlanner.resultOf` → `Platform.recipes()`，
  而本世代**从没注册过配方域**，未注册时那口**直接抛异常**——不装就是「玩家一放万能熔炉就崩」。
  故本刀建 `RetroRecipeAccess` 并**实装熔炼一域**（其余三域安静返回"没有"，随 ⑤d1 到序）。
  熔炼域的世代差是**查对照表**得的：1.20.1 `getAllRecipesFor` 直接返回 `List<SmeltingRecipe>`
  （1.21 是 `RecipeHolder` 包装）、id 走 `getId()`；候选三元组 `{配方id, 产物id, 产量}` 的形状
  与 Legacy 逐位一致——`SmeltPlanner.pickStable` 的稳定选序靠它，错一位两代就会选出不同配方。
- **路由两面零改动**：`accepts`/`chainWants` 的熔炉族分支在 m486 下沉路由脑时**已随主线原文
  一起搬过来了**，配方域一装即生效。这是真移植的又一份红利——**功能到序时不用再改路由**。
- **判官两条（累计六十五）**：①供料线路径端到端（仓里粗铁 →金线→ 万能熔炉 →绿线→ 产出仓出铁锭，
  同仓的泥土一件不动）；②不接线不取料（红灯「未接存储/供料线」）+ m149 白名单只烧勾选的。
- **验证**：两代冒烟经筛选器复查均无必须看的真错；20 闸全绿（0.1.494 对表）。零资源改动；
  **本刀装了配方域（新增运行期注册），动了 tick 路径**——实机重点验。
- **实机验证脚本**：①存储核心塞粗铁+泥土，点熔炉再点仓循环到**金线=供料**，另拉**绿线=产出仓**：
  应稳定出铁锭，泥土一件不少；②不接任何线时熔炉应红灯「未接存储/供料线」且**仓里什么都不动**；
  ③给熔炉设白名单只勾金矿：粗铁不再被烧；④工程款熔炉（mega_super_smelter）产量应是基础款 ×108。
- **下一刀**：B2 tick 执行面下沉——先办前置「统一两处世代取舍」。

## m495 B2 前置：产物分发下沉——「执行面取舍不同」原来只差三行

- **按作业表取 B2**（tick 执行面下沉），表里写着前置是「先统一两处世代取舍」。而 m473/m475
  两次记档都断言「执行面两代取舍确实不同，**不是照抄能解决的**」——这刀真去逐句对了一遍，
  **发现那个断言下得太早**。
- **两代的 `distribute0` / `routeOut`：循环体逐字相同**（m150 两轮垫底、m270 类型上限跳过、
  BUF_CAP 封顶投喂、垃圾桶族第二轮才喂），**唯一分岔是最后三行的兜底**——
  主线 `depositFor → depositOrBuffer → addOutput`（输出缓存 + 断网喷射），
  1.20.1 `depositTail`（落 kind0 产出仓，拒收原样回吐给调用方亮黄灯持料）。
- **所以「统一世代取舍」的正确形状不是逼两代行为一致**（主线有输出缓存、本世代没有，
  这是真实差异，硬统一等于给某一代加它不该有的东西），**而是把差异收窄到一个口**：
  新建 `ProductRouter`（共用两轮垫底与投喂）+ `Tail` 世代口（兜底各实现各的）。
  与前面十一个世代口同一手法——**第十二个口**。
- **顺带把 `isTrashLike` 收进共用件**：m150 垃圾桶 + m378 虚空处理器都是「最后去处」，
  第一轮一律跳过。本世代那份重写删除，判定改走 `NodeTags.defOf == Machines.VOID_PROCESSOR`
  （m472 身份口，原文是 `instanceof VoidProcessorItem`）。删完跑第 19 闸确认无悬空引用。
- **教训**：**「不是照抄能解决的」这种断言，下之前要先逐句对一遍**。m473/m475 我写下那句时
  是**从行为描述**推的（"主线有输出缓存、本世代没有，所以执行面没法共用"）——听起来成立，
  但真去看代码，不同的只有三行、相同的有二十行。**从行为差推代码差，会把可共用的部分一起判死。**
  这条对 B2 剩下的部分（tick 编排/六分支/拉料/灯表）同样适用：下一刀先逐句对，不预设结论。
- **验证**：两代冒烟经筛选器复查均无必须看的真错；第 19 闸（悬空引用/括号/资源）绿；
  20 闸全绿（0.1.495 对表）。零配置零资源改动，**动了两代的产物分发路径**（tick 热路径）。
- **实机验证脚本**：①机器→下游机器出线：产物应先喂下游缓存，喂满后落产出仓；
  ②下游接垃圾桶 + 一台耗料机：**沙子应全进耗料机、垃圾桶饿着**（两轮垫底）；
  ③下游全满且无产出仓：主线应进输出缓存（可从核心界面取出）、1.20.1 应黄灯持料且缓存数字可见；
  ④两代产量对账：仓里增量 + 缓存增量 = 产量读数。
- **下一刀**：B2 主体——tick 编排/六分支/拉料/灯表，**逐句对完再决定哪些能共用**。

## m496 B2 续：均分分发下沉——同一个形状的第二次兑现

- **照作业表的开工纪律办**（m495 立的："先逐句对，不要从行为差推代码差"）。这刀对的是
  **逻辑节点六分支**与**均分分发**：
  - 逐句对完六分支，确认两代的差异**仍然只是那一处**——主线的 `distributeEven` 是 `void`
    （有输出缓存兜底，永远送得出去，所以 `moved` 无条件为真），本世代返回剩余量、要回灌+亮黄灯。
    **循环体、清运节拍（5t）、m350 整锅转存再清、灯表语义全部同构。**
  - 于是 `distributeEven0` 也按 m495 的办法下沉：**均分主体共用**（m357 scratch 两遍法、
    `share`/`extra` 余数轮转、m270 类型上限拒收份额转兜底、BUF_CAP 封顶），**兜底走 `Tail` 口**。
- **两代的 `evenOk` scratch 同刀删除**（移进共用件了），删完跑第 19 闸确认无悬空引用。
- **这是 m495 那条教训的第二次兑现**：连着两块被我先前判为「执行面取舍不同、没法共用」的东西，
  真去逐句对之后**都只差最后三行**。**"行为描述不同"和"代码没法共用"之间没有必然关系**——
  前者说的是产品语义，后者要看的是代码结构，中间隔着一层，不能直接跨。
- **验证**：两代冒烟经筛选器复查均无必须看的真错；第 19 闸（悬空引用/括号/资源）绿；
  20 闸全绿（0.1.496 对表）。零配置零资源改动，**动了两代的均分分发路径**（分配器节点热路径）。
- **实机验证脚本**：①分配器接两台下游：产物应两边各一半，**奇数时前一台多一件**（余数轮转）；
  ②其中一台下游类型已满：它的份额应转给兜底（主线进输出缓存/存储、1.20.1 落产出仓或黄灯持料），
  **不许堵在分配器缓存里**；③拆掉全部下游：余量应落产出仓（本世代）或输出缓存（主线）。
- **下一刀**：B2 剩余——tick 编排骨架与拉料循环，继续逐句对。

## m497 修 m494 的构建失败 + import 完整性普查（第四类盲区）

- **作者回报 1.20.1 构建失败 3 个错误**：`找不到符号: 变量 Items` ——m494 那两条熔炉族判官
  用了 `Items.RAW_IRON`/`Items.DIRT`，而 `RetroTickTests` 没 import `Items`。补上 import。
- **为什么又漏了**：这是**第四类验证盲区**，与前三次形状不同：
  - m487 漏的是「**已删**的符号还有没有人用」→ 第 19 闸悬空引用；
  - m488 漏的是「切段漏带闭括号」→ 括号配平并入第 19 闸；
  - m491 漏的是「**类型错误**这一整类」→ 冒烟报告筛选器；
  - **本刀漏的是「**新写**的代码用了本文件没 import 的类型」**——它在 javac 输出里是
    `cannot find symbol`，而 m491 定的筛选器判据**把这一类一律归噪音**（缺 MC jar 时它有上千条，
    不归噪音工具就没法用）。**于是这个真错正好落在两把尺子中间的缝里**：
    筛选器认为它是噪音，第 19 闸只管已删符号，没人管它。
- **补法**：第 19 闸并入「**import 完整性普查**」。判据刻意做窄以求零误报——只认一张
  **常用 MC 类型白名单**（Items/Blocks/ItemStack/BlockPos/CompoundTag/BuiltInRegistries…），
  用了 `Xxx.` 或 `new Xxx(` 却既没 import、又不是全限定名、又不在同包、又不是本文件自定义，即红。
  **自证方式是重放本次故障**：把 `Items` 的 import 拿掉 → 闸当场红并指到行号 → 补回复绿。
- **第 19 闸现在管四件事**：悬空引用（m487）、括号配平（m488）、共用件资源（m489）、
  import 完整性（m497）。加上 m491 的筛选器，五件。**它们的共同点始终是同一个**：
  都发生在「我的验证」与「作者的构建」之间那道缝里。
- **值得记的是这次的形状变化**：前三次是「我的清单里没有这一类」，这次是
  **「两把尺子各自认为对方会管」**。工具多了之后，漏洞不再只出现在工具内部，
  **也会出现在工具的交界处**——所以每加一把尺子，都要问一句：**它和已有的尺子之间有没有缝？**
- **验证**：两代冒烟经筛选器复查均无必须看的真错；20 闸全绿（0.1.497 对表）。零配置零资源改动。

## m498 B2 收尾：拉料循环下沉 + 第二批对表闸退役

- **B2 最后一块大件**。照纪律逐句对完主线 m92 拉料段与本世代 `pullSupply`：
  **差异六处，全是参数化能收的，无一是"逻辑不同"**——①泵速倍数（主线 ×(1+数量升级)，
  本世代无升级恒 ×1）②精确账本源（主线可多源含数据面板聚合，本世代单源）③出线目标
  （主线 m355 数组化传入 vs 本世代现算）④饱和加法的宿主类名 ⑤合成机需求缓存（scratch 复用）
  ⑥m218c 多核心移相。全部收进 `SupplyPuller.Host`，其余逐字相同——**包括 m157 那段
  "猪人塔/幽匿线产物消失"的血案注释**。
- **白捡了 m480 的东西**：精确账本 bank 的类型直接用 `StorageLedgerProbe`——那个接口建它时
  是为跨代行为契约（m480），这里发现两代的存储核心都已实现，`exactTemplates`/`withdrawExact`
  现成可用。**为 A 目的建的抽象，在 B 处白捡**，这是真移植攒基础设施的复利。
- **第二批对表闸退役**：下沉完成后第 16 闸当场红，报「拉料五族清单漏抄 5 条」——
  因为 m475 立那条项时两代各写一份，靠尺子追平；现在两代跑同一份代码，**"漏抄"按构造不存在**。
  摘除并在闸文件里记档。**规律浮出来了：每当一块业务真移植完成，为它设的对表项就同时失去对象
  ——对表闸的萎缩速度，正好是真移植的进度条。** 剩余对表项只剩「已建方块名单」这类
  真·两代各写的东西。
- **账目**：业务双写 **1964 → 1902 行**。B 线主体（图状态/身份键/账本/路由脑/分发/均分/拉料）
  至此全部下沉，`StructureCore120` 剩下的是 tick 编排骨架与世代壳。
- **验证**：两代冒烟经筛选器复查均无必须看的真错；第 19 闸四项（悬空引用/括号/资源/import）全绿；
  20 闸全绿（0.1.498 对表）。零配置零资源改动，**动了两代的拉料热路径**。
- **实机验证脚本**：①仓→(金线)→过滤器(白名单=沙)→耗料机：沙被拉走、名单外的不动；
  ②抽取节点：默认关不抽 / 开了但没去处仍不抽（m157）/ 接产出仓后搬仓模式全抽；
  ③仓里放带附魔的剑 + 链尾接垃圾桶：剑应被抽走销毁；链尾换成别的机器：剑一件不动（授权闸）；
  ④泵挡位调高：缓存上限应随速率放宽（m163a），不再卡在 4096。
- **下一刀**：B2 真正的收尾——tick 编排骨架（预算/节拍/邻接派生位/灯表）逐句对，
  然后把 `StructureCore120` 在双写闸里降级为世代壳。

## m499 B2 收官：逻辑节点六分支下沉 —— StructureCore120 降为世代壳

- **B2 最后一步**。m496 逐句对过六分支，当时结论是「同构，唯一差异是主线 `distributeEven` 为
  `void`（有输出缓存兜底永远送得出去）而本世代要拿剩余量回灌」。m495/m496 把分发与均分下沉、
  **统一成都返回剩余量**之后（主线兜底恒回 0），这段就再没有拦路的差异了——本刀下沉。
- **六分支逐句照原文**：5t 清运节拍、m350 整锅转存再清、m150 垃圾桶吞光并累计"已吞"、
  **m153 抽取节点的残量留缓存=背压**（不亮黄，与其余五族不同，是原文刻意的）、
  开关/传感器关闸的"持料待命"词条。
- **灯表刻意留在世代口**：两代的灯表口径本就不同——本世代 m473 加了"持料待命"黄灯，
  因为它没有输出缓存、货真的会停在缓存里；主线有输出缓存不需要这条。**这是真实的世代差，
  不该硬统一**，收进 `Host.lampAfterDrain` 各实现各的。
- **`StructureCore120` 从 991 行降到 837 行，并在双写闸里正式降级为「世代壳」**：
  路由脑（m486）、分发（m495）、均分（m496）、拉料（m498）、六分支（m499）全部走共用件，
  本文件只剩 BE 注册 / 存档签名 / tick 编排骨架 / 宿主面 / 机器生产分支。
  **账目：业务双写 1829 → 991 行**（`StructureCore120` 那 838 行从欠账里划掉）。
- **B 线主体至此收官**。回头看，B 线开工时（m477）账上是 **2226 行业务双写**，现在 **991 行**，
  而剩下的 991 里有 309 是 `StorageCore120`（B 阶段做过一半：账本已共用，剩 FTA/漏斗/存档签名，
  多半属世代壳）、其余是数据线与面板（B3/B4）。
- **一条贯穿 B 线的观察**：m473/m475 我曾断言执行面「不是照抄能解决的」，之后 m495、m496、
  m498、m499 连着四刀逐句对，**每一次差异都只是参数化能收的东西**。四次之后可以下结论了：
  **"两代行为有差异"几乎从不意味着"代码没法共用"**——差异通常只住在很少的几行里，
  而把它们收进口，剩下的就能整段共用。这条对后面的 B3/B4 同样适用。
- **验证**：两代冒烟经筛选器复查均无必须看的真错；第 19 闸四项全绿；20 闸全绿（0.1.499 对表）。
  零配置零资源改动，**动了两代的逻辑节点清运热路径**。
- **实机验证脚本**：①六族逐个过一遍（分配器均分/过滤器放行拦下/垃圾桶吞光并涨"已吞"/
  抽取开关两态/传感器关闸持料）；②**抽取节点残量应留在缓存里且不亮黄**（背压，与其余五族不同）；
  ③下游全满时其余五族应亮"持料待命"黄灯且缓存数字可见；④退出存档再进，在途量不变。
- **下一刀**：B3 `DataPanel120` 下沉（190 行），继续逐句对。

## m500 B3：面板聚合两代共用 + 顺手修掉一条主线真编译错

- **按作业表取活**（B2 已收官，取 B3 `DataPanel120` 下沉），**开工先逐句对**（m495 立的纪律：
  不要从「行为描述不同」推「代码没法共用」）。对完的结论是这条线上第五次同一个形状：
  本世代那份 60 行 `snapshot` 是把主线**两步**压扁成一个方法的仿写件——主线是
  「BE 出全量快照（`masterEntries`）→ handler 出分页（`repage` 前半）」，本世代是一个方法从头做到尾；
  **循环体、排序意图、服务端钳位口径本就同构**。
- **真差异三处，全部是「本世代少了主线的加固」**，一处世代取舍都没有：
  ①**精确条目跨核心不并账**（主线按「物品+附加数据」并成一行，本世代每核心每模板各占一行——
  两台核心里各有一本同款附魔书，面板上会分两行各显各的数）；②**排序尾键缺两级**
  （主线四级：量↓/id↑/普通在前/数据串稳定；本世代只有两级，同量同 id 时序不定=刷新抖动）；
  ③**大小写归一**主线用默认 locale、本世代用 `Locale.ROOT`（**这处反而是本世代对**：
  土耳其 locale 下 `"I".toLowerCase()` 会变成 `ı`，id 匹配当场断）。
- **落地**：新建 `xplat/storage/PanelAggregator`——`entriesOf`（普通聚合+精确并账+m83 存量降序）、
  `page`（过滤/钳滚动/开窗）、四口聚合读写门面，**全部主线原文整段搬**（含 m130/m267/m292/m322 注释刀号）。
  两代各删各的：本世代 `snapshot` 主体删除改转发、主线 BE 的 `DispEnt`/`MASTER_ORDER`/聚合循环退役改转发。
  合一后三处加固自动推广（m477 红利同款），行为变更配判官一条
  （`panel_snapshot_merges_exact_entries_across_cores`：跨核心并账=11 + 同量同 id 普通在前）。
- **核心列表直接吃 `StorageLedgerProbe`，不另开接口**：起初想建 `PanelAggregator.Core`，
  写到一半发现两代存储核心**早就实现了 m480 那个探针**（`storeView`/`exactTemplates`/`exactCount`/
  `deposit`/`withdraw`/`withdrawExact`/`count` 全在里面），于是共用件直接吃 `List<? extends StorageLedgerProbe>`，
  **两代零 implements 改动**。这是 m480「为跨代行为契约建的抽象」第二次被白捡（第一次是 m498 拉料的 bank）——
  **真移植攒基础设施的复利是实的：为 A 目的建的口，在 B 处不用再建一遍。**
- **一个新世代口（第十三个）**：`MASTER_ORDER` 的最后一级尾键用 `getComponentsPatch()`（1.21 专属，
  m487 血案同款符号），收进 `StackKey.Kind.dataOrder`——两代各写一句原文（组件补丁串 / tag 串），
  与 `same`/`dataHash` 同口径（same 为真必然本串相等，排序与相等判定不打架）。
- **刻意不搬的**：展示栈**物化**（主线 `ResourceLocation.parse`+写 54 个真槽位，本世代 `tryParse`+虚拟列表包）
  与三套缓存（cores 40t / viewCache / masterCache 指纹）——前者是协议形状差（m485「留世代壳各写」同律），
  后者**只有主线有观众态与多观众场景**，不为接口好看把只有一边用的东西搬上来（m480 同律）。

### 顺手抓到的：一条**主线**真编译错（本刀最该看的部分）

- 冒烟经 m491 筛选器复查，报出 9 条「必须看」：`SdzjzGameTests` 在 `com.sdzjz.gametest` 包里
  直接 `be.g.machineNodes`（9 处），而 `StructureCoreBlockEntity.g` 是**包私有**——
  `g is not public ...; cannot be accessed from outside package`，**主线根构建会红**。
- **先清自己嫌疑**（m142 家法）：`git stash` 跑基线对照组，HEAD 同样 9 条 → 不是本刀捅的，
  是 m481 那条 `route_domain_contract` 判官带进来的（m430 mA1 把图状态搬进 `CanvasGraphState`
  时字段留了包私有，判官在另一个包里直接摸字段）。按取活规则第 1 条插队修掉：与邻居 `prof`
  同口径放开为 `public`（**不为判官造新访问器**——不造新接口，先复用已有）。
- **教训（这刀真正的收获）：尺子的量程也是尺子的一部分，而「源集」就是冒烟的量程。**
  m471 撞过一次（javac 默认只报前 100 条，拿截断清单跑「报错=0」等于没验），这次是同一个病的
  另一个器官：**前几刀我报「两代冒烟无必须看的真错」，量的是更窄的源集**（改哪代跑哪代），
  于是主线判官里这条访问错在 m481~m499 十九刀里一直没人看见。m497 说「每加一把尺子要问它和已有尺子
  之间有没有缝」，本刀补一句：**同一把尺子，量程换了就是另一把尺子**——所以冒烟的源集从今往后
  **两代都按各自 build.gradle 的真实源集全量跑**（1.20.1 侧白名单现读不手抄，m490 家法）。
- 附带说明：这条错说明**主线根构建至少从 m481 起就没被真正构建过**（作者近期实机与构建反馈都在 1.20.1 侧），
  HANDOVER D2「主线 Gradle 构建从未由我验证过」这条欠账不是形式主义，它今天兑现了一次。

- **账目**：业务双写 **991 → 978 行**。**数字小要说实话**（m482 教训：账目口径错了比没有账目更坏）——
  真删掉的业务双写约 55 行，但双写闸按**整文件行数**计账，而 `DataPanel120` 剩下的是
  BE 开屏三方法 / `PanelMenu120` / payload 处理这些**真·世代壳**加上一块 `serverTake`；
  **要等 B3b 把 `serverTake` 收完，这个文件才能整体降为世代壳、190 行的账才归零。**
- **`serverTake` 逐句对的结论（不下断言，留 B3b）**：两代**不变量完全相同**
  （取多少给多少、`Inventory.add` 塞不下的余量原路回账本绝不落地），差的是**循环结构与申报策略**——
  主线按 `maxStack` 分块 + 按钮档位（1/8/16/32/64/2组/4组/8组/填满），本世代按核心遍历 + 九栈钳位。
  按 m495 教训**不写「不是照抄能解决的」**（那句话我已经在 m473/m475 上栽过一次），B3b 再对一次再决定；
  其中「申报策略」若要统一属产品口径，汇报里已问作者。
- **验证**：两代冒烟经筛选器复查均无必须看的真错（主线由 9 条真错 → 0）；第 19 闸四项全绿；
  20 闸全绿（0.1.500 对表）。零配置零资源改动，**动了两代的面板展示路径与主线取物读账路径**。
- **实机验证脚本**：①两代各开面板，库存显示与排序应与之前一致（存量降序、同量按 id 稳定、不抖动）；
  ②**1.20.1 重点**：两台存储核心里各放一本**同款**附魔书 → 面板应显示**一行合计**（合一前是分两行）；
  再放一本**异款**附魔书 → 另起一行，两行数字各自正确；③搜索框输入 `cobble`（大小写混打）
  应同时命中裸圆石与带 tag 的圆石；④滚动到底再往下滚，行号应被服务端钳住不越界；
  ⑤shift 点背包物品入仓仍正常（带附魔的进精确条目、不混堆不变裸）；
  ⑥主线侧：面板右键浮层批量取出、清空回仓、m289 库存摘要（合成终端配方补料）三条照旧。

## m501 B3b 收官：取物回账下沉 —— DataPanel120 降为世代壳

- **B3 的后半**。m500 记档时把 `serverTake` 的逐句对结论写成了「两代**不变量完全相同**，
  差的是循环结构与申报策略」，并**刻意没写「不是照抄能解决的」**（m473/m475 栽过那一跤）。
  本刀真去搬，验证了那个克制是对的：**不变量确实是同一条，形状差全部可以留在调用点**。
- **差异的正确切法**：两代的差不在「取」而在「申报」——主线客户端发的是**档位下标**
  （1/8/16/32/64/2组/4组/8组/填满背包，服务端按 `maxStack` 算出 want），本世代客户端发的是**数量**
  （服务端按「一请求至多九栈」钳位）。**这是协议形状差，不是业务差**；于是共用件
  `PanelAggregator.takeInto` 只吃**算好的 want**，档位算法留主线调用点、九栈钳位留本世代调用点。
  （作业表阻塞区那条 A/B 走的 A：不动任何玩法手感、不动协议。）
- **搬进来的循环体是主线 m82/m100 原文**：按堆叠上限分块 → 跨核心累计取 → `Inventory.add` →
  `given += got - give.getCount()` → 塞不下的余量 `deposit` 原路回账本（m130 带附加数据的自动分流回精确账本，
  不混堆不变裸）→ 仍非空则 `player.drop` 双保险 → 见底即停。m112 客户端保险丝随循环一起搬
  （原住 BE 门面，共用件改判 `player.level().isClientSide`——它才是这条调用链的权威侧）。
- **本世代白捡两处加固**：①**按堆叠上限分块取**（原为一次申报全量，576 个一口气 withdraw）；
  ②**余量回账本后仍非空则 drop 双保险**（原来只回账本，回不进去就静默丢——虽然理论上回得去）。
  主线白捡一处：分块的 `maxStack` 从 `Item.getDefaultMaxStackSize()` 改成 `ItemStack.getMaxStackSize()`
  ——**m163c「全库动态堆叠上限走栈不走 Item」那条家法在此处一直没落实**，合一顺手对齐（大堆叠模组下才有差）。
- **`DataPanel120` 在双写闸里降为世代壳**：本文件只剩 BE 开屏工厂（`ExtendedScreenHandlerFactory`
  两代签名不同，对照表已记）、虚拟列表菜单 `PanelMenu120`（主线是 54 展示槽+合成网格 handler，协议形状差）、
  展示栈物化、申报量钳位、payload 处理——**都是本世代协议侧该各写一份的东西**。
  **账目：业务双写 978 → 801 行**（B3 整刀 991→801）。B 线只剩 `StorageCore120` 剩余 309 行与 B4 数据线两件。
- **判官 +1**（累计两刀 +2）：`panel_take_spans_multiple_cores`——两台核心各存 40/24，一次取 64 应取满、
  两边账本合计扣净。**它钉的是不变量不是形状**：合一前是「按核心遍历+一次申报全量」，合一后是
  「共用门面累计取+分块」，判官对两种形状都该绿——**这正是判官该有的写法**（钉不变量的判官在重构时不用改，
  钉形状的判官会逼着你把已合一的代码再拆回去喂它，m486 退役第一批对表闸就是这个道理）。
- **教训（把 m495 那条往前推一格）**：m495 说「不要从行为描述推代码差」，本刀补上后半句——
  **逐句对完发现差异之后，还要再问一句「这个差异住在哪一层」**。`serverTake` 的差异看着在业务层
  （循环结构都不一样），实际住在**协议层**（客户端发档位 vs 发数量）；判断标准很简单：
  **把差异挪到调用点之后，剩下的部分还对得上原文吗**——对得上，就说明差异本来就不属于这段业务。
  B 线一路下来五刀，差异分别住在「兜底口」「灯表」「参数」「协议」，**没有一次真的住在业务逻辑里**。
- **验证**：两代冒烟（**按 m500 新口径的全量源集**）经筛选器复查均无必须看的真错；第 19 闸四项全绿；
  20 闸全绿（0.1.501 对表）。零配置零资源改动，**动了两代的面板取物路径（碰账本与玩家背包）**。
- **实机验证脚本**：①两代面板右键浮层九个档位逐个点（1/8/16/32/64/2组/4组/8组/填满），
  到手数与账本扣减应逐个对得上；②**背包只留一格空**再点「填满背包」：应只进得去那一格的量，
  **账本余量分毫不差**（绝不落地、绝不消失）；③**跨核心取**：两台存储核心各放半组同款物品，
  取一整组应一次取满、两台都扣；④**精确件**：取附魔书应保附魔（不变裸不混堆），背包满时余量回仓仍是精确条目；
  ⑤本世代虚拟列表点取物，客户端申报天量（改包）应被钳在九栈内；⑥shift 从展示区取物（主线路径，本刀未动）照旧。

## m502 B4a：数据线抽取口业务下沉 —— m440 旧结论过期，五口门面本世代兑现

- **先逐句对，再问差异住在哪一层**（m501 教训第一次当工序跑）。两代数据线 BE 对下来：
  m225 逐模板抽取 / 全部模式游标轮转 / m231 回收拍 / m228 六面视图 / m218b 核心 40t 缓存，
  **业务逐位相同**；真差异三类——①transfer 触点（主线走 Xfer 门面，本世代 m444 内联 FTA）
  ②主线专属功能（m229 转化桌 EMC / m230 升级槽 / m233 按面断开）③附加数据判定（组件 vs tag）。
  三类分别住在**加载器层 / 功能层 / 世代口**，又一次没有一个真住在业务里（B 线第六刀，六比零）。
- **①的解法是把 m440 的旧结论作废**：m444 排刀稿写过「本世代不设 Xfer 门面，transfer API 直接内联」
  ——那是仿写路线的取舍；而 m434 把 Xfer 接口化时注释里**本来就预言了**"1.20.1 格照此同款
  （该版 fabric-api 同有此 API）"。真移植路线下这五口门面**本身就是世代口**：新建 `RetroXfer`
  （**FabricXfer 原文照搬**，一行未改）在 RetroBootstrap 安装，1.20.1 侧的 transfer 内联段全部退役。
  **教训（m483 同型再犯一次就该立规矩了）**：旧结论要定期重量，特别是「路线切换」之后——
  m440 那句话在它写下的那天是对的，路线从仿写换成真移植的那天它就过期了，但没人回头把它划掉。
  **今后凡引用 m4xx 排刀稿结论，先问一句「它预设的路线还在吗」。**
- **②的解法是带默认值的宿主口**（ProductRouter.Tail 同款形状）：`ExtractPort.Host` 上
  faceDisabled/isSellNeighbor/sellerActive/sellOverflow 四钩子默认关，本世代零实现即得蓝本行为；
  主线 `PortHost` 内部类兑现四钩子（sellOverflow=原 insertInto 卖桌尾段**原文**）+ 核心网络三口。
  ③走现成世代口：exact 判定=ItemData.has（m487 血案同款替换）、精确匹配=StackKey.same
  （m502 新增静态出口，本世代实现即原 isSameItemSameTags，逐位不变）。
- **共用件 `ExtractPort`**（xplat/storage，约 240 行）：实例态七件（extractOn/filter/rrCursor/
  opTargetsFull/coresScanTick/coresCache/pullMode）+ scanAdjacent/cores/runPulse/extractSpec/
  extractAll/doPull/pullWants/insertInto 全部主线原文（注释刀号随迁）。零 fabric import（layer_gate ①）。
- **两代各白捡一处**：本世代拿到 extractAll 的 **tryParse 防御**（原主线 parse 会在坏档怪 id 上抛异常
  炸整拍，m443 给账本补过、抽取口一直没补——两代同名静态，合一顺手对齐）；本世代 insert 少了一层
  「supportsInsertion 先查再插」的冗余守卫（FabricXfer 原文没有；FTA 契约下 insert 对只出不进的
  存储恒返 0，等价）。**主线 NBT 键逐字节不动**（extractOn/owner/pullMode/offFaces/upN/filter），
  本世代键同名同布局照旧——**存档双向兼容，零迁移**。
- **两代壳收尾**：主线 BE 323→211 行（六个私有方法整段退役，剩宿主口+m229/230/233+开屏工厂+linkBlocked）；
  本世代 288→143 行并在双写闸**降为世代壳**（剩 BE 注册/存档签名/tick 相位闸/m469 端点自愈/
  m449 持物右键过滤器交互）。**账目：业务双写 801 → 512 行**。B 线只剩 B4b 数据线方块（203 行）
  与 StorageCore120 剩余 309 行。判官零新增：本世代五条数据线判官（送出/回收/白名单/九栈守恒/tag 件）
  全部原样跑在共用代码上——**判官钉不变量的红利第三次兑现**（m486/m501 同理，重构零改判官）。
- **验证**：两代全量源集冒烟经筛选器复查零真错；20 闸全绿（0.1.502 对表）；第 19 闸新登记两代数据线
  删除符号清单。零配置零资源改动，**动了两代数据线的抽取/回收路径（碰账本、邻接机器与 EMC）**。
- **实机验证脚本**：①抽取口贴箱子开抽，仓内物品按游标轮转进箱、拍频与批量吃配置；②白名单持物右键
  加圆石后只抽圆石，再点移除回全抽；③m231 回收模式从 Create 传送带/漏斗收货进仓，带 tag 件入精确账本；
  ④目标箱满：余量回账本绝不落地，本拍收工下拍续；⑤主线专属三件照旧——转化桌贴桌溢出卖 EMC 记
  所有者账、升级槽提速提量、链接器按面断开后该面不对接；⑥两代旧档读入：开关/方向/白名单原样恢复。

## m503（接续 m502）B4b：数据线方块层下沉 —— CableEnd120 退役，B 线数据面板/数据线两大块收官

- **B4 收尾**。逐句对两代 `DataCableBlock`：属性表（六个 `EnumProperty<CableEnd>` + `END_PROPS`
  映射）、碰撞箱几何（`CORE`/`ARMS` 十二个 box 坐标 + `SHAPES` 缓存）、三态判定核心（`endFor` 的判定树：
  本端断开→NONE / cable 邻居按对端断开出 NONE-或-CABLE / 名单命中→PLUG / Container→PLUG /
  特殊出售邻居→PLUG / 暴露传输平台口→PLUG / 其余 NONE）**逐位相同**；真差异只有两类——
  ①`CODEC`/方法可见性（1.20.3 才有 `MapCodec`，1.20.5 起 Mojang 才把 `getShape` 等收紧成
  `protected`，纯版本 API 差，`override` 本身没法下沉）②主线专属功能 m233 按面断开 / m229 转化桌
  （与 B4a 同款差异，带默认值钩子接住）。**又一次没有一个真差异住在业务里**（B 线第七刀）。
- **`CableEnd` 与 `CableEnd120` 枚举本就该是一份**：两者序列化名 `none/cable/plug` 逐字同值，
  零版本相关 API（只依赖 `StringRepresentable`），当年分两份纯属仿写路线的历史遗留——
  `CableEnd120` **整个文件退役**，1.20.1 直接用 xplat 的 `CableEnd`。
- **新共用件 `CableEndCore`**（xplat/block）：属性表/几何数据/`shapeFor`/`addStateDefinition`/
  `allNone`/三态判定核心 `classify` 全部主线原文整段迁入。**`classify` 的判定钩子刻意设计成
  跟 B4a 的 `ExtractPort.Host` 同一副面孔**（`selfFaceDisabled`/`neighborFaceDisabled`/
  `isSellNeighbor` 三个断开与出售相关的布尔，本世代调用点恒传 `false`）——同一套"主线专属功能
  用默认关的钩子接住"手法在 B 线里第三次兑现，不是巧合是套路验证有效就该重复用。
- **"自家网络方块名单"这份数据，刻意没有跟着下沉**：两代名单字面来自不同注册类
  （`ModBlocks` 六块 / `RetroBlocks` 三块），`tools_retro_parity_check.py`（第 16 闸）靠**抓
  `endFor` 方法体里的 `前缀.常量` token 文本**防漏抄（m469 血案：漏抄一条不报错，判官照绿，
  只有眼睛能看出来）——若把名单判断包进一层新的私有方法，闸的"取方法体"函数只会抓 `endFor`
  自己花括号内的文本，抓不到被调用方法里的常量，会让这条判据抓到 0 条名单直接报错退出。
  **两代 `endFor` 因此保留為"局部变量判成布尔 + 一行转发 `classify`"的极简形态**，名单字面
  一个字都没挪窝，第 16 闸改动前后抓到的名单数量/内容逐位不变（蓝本 6 条/本世代 3 条，
  豁免 3 条未建方块）——**闸的判据依然是真实生效，不是靠文本巧合虚绿**，这条本刀特意验证过。
- **两代壳收尾**：主线 `DataCableBlock` 153→118 行（`NORTH`/`SOUTH`.../`END_PROPS` 保留为
  转发字段，外部引用点如 `DataCableRenderer` 零改动）；`DataCableBlock120` 202→161 行并在
  双写闸**降为世代壳**（外部引用点 `DataCableRenderer120`/`RetroStorageTests` 改走完全限定名
  `CableEndCore`/`CableEnd`，两处共三行改动）。**账目：业务双写 512 → 309 行**。
  **B 线数据面板（B3）与数据线（B4）两大块至此全部真移植完成，只剩 `StorageCore120` 剩余
  309 行**——B 线账目里"两代各写一份"的业务代码，此刻只剩存储核心账本本体这一件事。
- **验证**：两代全量源集冒烟经筛选器复查零真错；第 16 闸单独复核（本刀改动前后名单抓取
  内容逐位不变，判据真实有效不是虚绿）；第 19 闸补登记两方块类本刀删除的十三个符号
  （主线 `SHAPES`/`CORE`/`ARMS`；本世代同三个外加六个 `EnumProperty` 字段与 `END_PROPS`）；
  20 闸全绿（0.1.503 对表）。零配置零资源改动，**动了两代数据线的方块状态判定与渲染取值路径**。
- **实机验证脚本**：①两代数据线摆放/邻接自家三大方块（存储核心/结构核心/数据面板）应各伸
  一只插头，缆对缆应连续无接头盒；②贴普通容器（箱子）/暴露 FTA 的第三方存储（Create 置物台等）
  同样伸插头；③线对线直角转弯外观正确（碰撞箱与视觉一致）；④主线专属：链接器潜行右键断开某面后
...
## m505 A4/A5 小普查（零代码笔）：厘清依赖、发现服务端分组业务可真移植

- **B 线业务代码 m504 收官后转 A 线**。作业表里 A4（连线归并徽章 m193）与 A5（分组框选
  m192）互相把对方列成依赖——动手前先厘清方向。主线代码本身给了答案：`StructureCoreScreen`
  第 590 行原注"m193 分组共享表一次算好：组成员/组框矩形/节点→组查表（组框渲染与连线归并
  共用）"——归并的锚点算法是"端点属组→锚到组框缘"，没有组就没有锚点。**A5 先于 A4，作业表
  原表述是笔误**（历史某次编辑把两行依赖列反了），本刀订正。
- **意外发现：服务端"分组"业务是又一次 B 线故事**。逐句核对：`CanvasGraphState.groupNames`
  （组号→组名）与它的存档读写从 m477（图状态两代合一）起就已共用；成员归属走
  `NodeTags.nodeGroup`，`NodeTags` 整个文件 m472 起也已共用；本世代 `StructureCore120`
  已经有同一个 `g`（`CanvasGraphState` 实例）字段。真正没共用的是六个业务方法
  （`createGroup`/`dissolveGroup`/`renameGroup`/`moveGroup`/`setNodeGroupTag`/
  `sweepGroups`，约 75 行）——逐句读下来**零 MC 1.21 专属 API**，只碰已共用的类型，
  跟 BE 生命周期打交道的只有 `setChanged()`/`syncToClient()` 两个调用点，跟
  `StorageLedger` 那套 `Runnable onChange` 解耦手法是同一个形状。
- **同步机制两代不同，但那是执行面差不是数据差**：主线走"服务端脏检测+版本号+观众登记表+
  按需推送"，本世代走"客户端主动 `CanvasQuery` → 服务端当场编码 `CanvasSnapshot` 回发"
  （无观众态）。两者都是把 `CanvasGraphState`（含 `groupNames`）编码进快照——**分组数据
  只要写进 `groupNames`，两代现有快照通道原样能带到客户端**，不需要另开读协议。同 m480
  记档"只有主线有观众态，不为接口好看造对位"一律：执行面不同不代表数据不能共用。
  **结论：服务端真正缺的只是"客户端发起写请求"这一条通路**——新网络包（对位
  `NodeGroupPayload`/`NodeGroupMovePayload`，走 `FabricPacket` 样板照抄
  `NodePayloads120.java`）+ 接收器，不需要另建数据结构。
- **客户端是真正的大头，且不可共用**：1.20.1/1.21 的 `Screen`/`GuiGraphics` API 差异大，
  主线 m192/m193 相关代码高度耦合这个 Screen 类自己的一整批状态字段（`selected`/
  `dragGid`/`boxSelecting`/`renameGid` 等），不是能抽出纯函数的形状。粗量：`m192`/`m193`
  标记在主线各出现 22/8 处，分布在渲染方法（~200 行区间）/辅助方法（~150 行区间）/
  鼠标事件穿插判断（12 处）/键盘事件穿插判断（6 处），且**大多"夹在"更大的通用方法里**——
  `connectedComponent`/`hoveredNode`/`openPrimaryPicker` 就恰好夹在分组辅助方法那段
  区间里但根本不是分组专属，验证过一次才敢确认边界（m487 血案同款教训：按行号盲切前必须
  先看区间里有什么）。两代客户端代码量粗估 300~450 行，本世代要新写不是共用。
- **产出**：`docs/画布分组归并1201_A4A5小普查_m505.md`——依赖方向订正、服务端/客户端
  两半的真移植可行性判断、区块行号粗量、四刀分片方案（A5a 服务端下沉 → A5b 客户端读+
  组框渲染 → A5c 客户端交互 → A4 归并徽章）、每刀验收口径。作业表 A5 拆成 A5a/A5b/A5c
  三行，依赖链改成 A5a→A5b→A5c→A4，A7 依赖也从 A5 改成 A5c。
...
## m507 A5b：画布分组组框渲染两代共用 —— 1.20.1 第一次画出组框（只读不写）+ 顺手修 1.20.1 命中框 24×24 残留

- **按作业表取活**（A5b）。m505 普查稿写的是「`CanvasScreen120` 新增组框渲染……本世代新写不是共用」——开工先
  照流程逐句对主线 `StructureCoreScreen` 的 m192 组框段（原 601~622）与几何助手 `groupMembers`/`groupRect`
  （原 1340~1363），结论与普查稿**相反**：这两块**世代差为零**（全是 `ctx.fill`/`SciSkin.hGrad`/`drawString`
  与算术，`PoseStack.translate(DDD)` 经 yarn 1.20.1 映射核过在位），真正耦合 Screen 状态的只有五个读口
  （坐标含拖动覆盖 / pan-zoom 记法 / 卡包围盒高 / 组元数据 / 拖动中的组）。**于是照 m484/m488/m490 的路数整段
  搬成 xplat `client/GroupFrameRenderer`**（`GPAD/GBAND` 原值、`groupMembers`/`groupRect`/`drawFrames` 三件），
  五个读口收进 `View`（坐标记法照 `MinimapRenderer.View` 同约法：视口左上画布坐标+缩放，换算在 View 做一次）。
  三段搬入体归一机械替换后**与主线原文 diff=0**（脚本对拍）。普查稿那句"不可共用"对渲染这半判得太早——
  m495 教训的 UI 版：**从"两代 Screen API 差异大"推"绘制件没法共用"，把纯 fill+算术的部分一起判死了**；
  A5c 的交互面（事件签名/EditBox/状态机）才是真不可共用的部分，那句留给它。
- **唯一参数化的真世代差**：`View.cardHeight()`——成员卡包围盒高。主线是「卡体+升级格行」`NH+26`（原注
  "与悬停判定同口径"），1.20.1 无升级系统（m464 记档）卡下没有那一行=`NH`；不参数化会给 1.20.1 组框底边
  多空 26 格。主线侧：渲染段换成共享表装配+一行 `drawFrames`（`nGid` 仍留主线喂 m193 归并），两几何助手
  改转发（`groupOfNode`/右键标题带/拖动标题带三处调用点零改动），`GPAD/GBAND` 留别名，新增 `groupView`
  视图口（`wnx/wny` 含 m196 拖动覆盖原样进口）；3177→3164 行。切段前先断言区间只含预期两件（fill 计数=6、
  无 seBundles，m487 血案：切前看区间里有什么）。
- **1.20.1 侧**：`CanvasScreen120` 加 `groupView`（坐标含拖动幽灵位=主线 wnx 的 m196 语义；`groupNames` 读
  快照像 `g.groupNames`——m506 服务端下沉后随 `CanvasSnapshot` 原样到；`dragGid` 恒 -1 待 A5c），`renderBg`
  网格之后、存储线之前插组框（**最底层**，主线层序同位），受 `canvasGroupsEnabled` 总开关。**本世代原来
  完全没有组框，这刀从零补上且与主线同一份代码**。
- **顺手修一个 1.20.1 潜伏 bug**：`nodeAt` 命中框还是 m484 之前的 **24×24**，而卡早已长成 NW×NH=100×52——
  只有卡左上角一小块能悬停出详情/按住拖动/右键摘回。m488 记档修过同族的「连线锚点按旧 24×24 卡算」，
  命中框这处漏了（视觉层改动时"命中框/坐标一律不动"是 m207 的纪律，但这次卡本身变大了，命中框该跟）。
  改成 `NW*zoom × NH*zoom` 整卡命中，与主线 `hoveredNode` 的 NW×NH 口径一致（主线另有 +26 升级格行）。
  放置模式的 `-12` 居中偏移同属 24 残留但只是落位手感，本刀不动。
- **判官零新增**：`GroupFrameRenderer` 在 `client` 包，GameTest 跑在服务端，判官从不引用 client 类（全库
  0 处先例——加载会碰 `GuiGraphics`/`PoseStack`）；几何正确性由 diff=0 对拍 + 主线既有行为兜着。
- **验证**：两代全量源集冒烟经筛选器复查零真错、自家新符号 symbol 级报错两代各 0、三段归一 diff=0、
  20 闸全绿（0.1.507 对表）。零配置零资源零协议改动，**纯 UI，不碰存档与运行时路径**。
- **实机验证脚本**：①主线：建组后组框/标题带「组名 ×N」/拖组提亮与之前逐位一致（同一份代码，diff=0，
  但共用件内做了 pose 变换，看有没有错位）；②主线：关 `canvasGroupsEnabled` 组框消失；③**1.20.1：本刀客户端
  仍无建组交互，组框只能等 A5c 建出组来看**——本刀实机只需确认画布照旧+**整张卡任意位置**都能悬停出详情/
  按住拖动/右键摘回（命中框修复，这条是本刀 1.20.1 唯一可见变化）；④1.20.1：缩放后命中框随卡缩放。
...
## m510 修构建失败（取活规则第 1 条）：主线编译错潜伏 28 刀 + 1.20.1 判官九条恒失败 —— CI 红灯自 m473 起没人读 + 第 21 闸

- **来路**：m509 推完照规矩轮询 CI，三个 job 红（Gradle 编译出包 / GameTest 用例集 / 1.20.1 编译+GameTest）。回溯 60 次 run：
  **CI 最后一次全绿是 m472**；1.20.1 GameTest 自 **m473** 起红、主线编译自 **m481** 起红，此后 37 刀次次红着推，没人读——因为沙箱
  拿不到 artifact 域名，而 HANDOVER 里"推完轮询 CI、闸红即停"的规矩在 m473~m509 间实际上没有人执行过。**但 CI 早就把错误清单
  回推到分支了**（m414 `ci-mojmap-errors`／m439b `ci-retro-errors`／m310b `ci-gametest-report`），raw 域可读；本刀就是从这三个
  分支读出来的。**教训一：工具早就在，用它的人没有——"读 CI"必须是每刀收尾的固定动作，读法就是 fetch 这三个分支**（作业表流程补第 11 步）。
- **主线编译错（唯一一条，`StructureCoreBlockEntity:3793`）**：`probeChainWants` 给 7 形参的 `chainWants0`（m355 outT 数组化+m350
  crafterNeeds 备忘，一直是七参）只传 5 个实参——m481 立路由域探针时按 1.20.1 五参口径写的转发。修法=补 `probeOutT()`（与 tick
  编译执行计划 m179/m355 同一份两趟算法但不碰缓存：计划新鲜就直用，否则现算——GameTest 无 tick 也要拿到当前拓扑；照 1.20.1 探针
  `outTAll()` 同款）+ 备忘表现配。**为什么两代冒烟、m508 逐文件单编、m504 "孤立编译精确复核"全部零真错**：`(Level) level` 类型不可解析时，
  javac 对该调用只报 cannot find symbol，**不再做重载适用性判定**——"cannot be applied to given types"在无 MC jar 下根本不会被报出来。
  这是**冒烟第六类盲区**，形状与前五类不同：前五类（m487 已删符号/m488 括号/m491 类型错/m497 import/m508 归因顺序）都是"报了没人看见"，
  这一类是"**根本不报**"，任何筛选器、任何排序、任何单编都救不了，只能靠数。
- **第 21 闸 `tools_arity_check.py`（裸调用实参个数）**：本文件只声明一次、无重载、非变参的方法，被本文件裸调用（`name(`/`this.name(`）
  时实参数必须等于形参数。判据刻意做窄（有重载不数、变参不数、`obj.name(`/`super.`/`new Name(` 不数、大写名不数），剥注释与字符串后按括号
  深度数逗号。**尺子修了三轮才立起来**：①首跑 14 条命中 13 条误报——包私有声明（无修饰符）没被认成重载、`chk(a < b, "…")` 的比较符被当
  泛型深度、`new Recipe(` 构造器没排除；②匿名类一行写完的 `@Override public int nodeCount() {…}` 没被认成声明；③**血案重放居然绿**——
  "声明位置集合"收的是 DECL 正则全部原始命中，`return chainWants0(` 那行（类型位=return，本该被排除）也进了集合，真调用被当声明跳过。
  改成只收真被认作声明的位置后：全库 227 文件 / 2561 处裸调用零误报；**自证=把 probeChainWants 退回 m481 五参写法当场红点名
  3798 行，还原复绿**（m506 教训：自证要用它声称能防的那次血案重放——这回第一遍重放就抓到了尺子自己的洞）。挂 CI 第 21 步、作业表命令加一项。
- **1.20.1 GameTest 九条恒失败（自 m473，判官从没真跑过——m479 早写了"判官没真跑过"）**，从 `ci-retro-errors` 服务器尾日志逐条读断言文案：
  ①**六条同一个根因**：`addNode` 存的是 `copyWithCount(1)` 的**副本**（生产语义：从背包拿一台上画布，tag 深拷贝），判官在加节点**之后**
  改本地引用的 tag（关开关 `so`/开抽取 `xo`/改白名单 `fl`/切传感器 `sl`）等于改了个孤儿栈——pair_switch/logic_leftovers/extractor_pump/
  smelter_whitelist/route_domain_contract（开关+传感器两处）全是这个形状；修法=加 `add(c, s)` 助手取回图里那份，六处改用。
  ②`exact_pull_requires_trash_terminated_chain`：m486 下沉的主线 `chainEndsInTrash` **沿途尊重过滤器名单**，空名单=全拦→链永远到不了
  垃圾桶（m473 自己那份路由脑不查过滤器，判官按它写的"白名单空=只留精确支路受检"前提已随真移植失效）；改成放行剑 id——普通账本里没有裸剑，
  普通支路照样一件不动，只开精确支路的路。③`distributor_even_split_remainder_cap_reroute`：拿骷髅头验"没人收"，但 5 号 wither_killer
  输入表含骷髅头且已持该类型不撞类型顶，本就收；改用两台都不吃的烈焰棒验。④`tick_lamps_without_output_storage`：期望熔炉族黄灯"待分片"，
  m494 熔炉族 tick 落地后未接线=红灯「未接存储/供料线」（smelter 判官同口径），期望随 m494 更新（m494 漏改了这条）。
  ⑤`stack_key_hash_contract_retro`："空 tag 在场 ≠ 无 tag"用钻石剑验——**1.20.1 `ItemStack` 构造器对可损耗物品会 `setDamageValue(0)`
  自带 `{Damage:0}`**，两把"裸剑"tag 同为 `{Damage:0}`，"无 tag"前提根本不成立；换不可损耗的石头验（1.20.1 `isSameItemSameTags`
  对 null vs `{}` 判不同，断言本身成立）。**九条全部是判官写错，不是业务错**——这既是好消息（业务没坏）也是坏消息（九条判官三十多刀里
  一直在"作证"却从没开过庭）。
- **教训二**：判官写完必须真跑一次才算存在——沙箱跑不了，那"真跑"就是 CI，**CI 红=判官不存在**。m479 说过"判官没真跑过"，之后 30 刀
  照样在 DEVLOG 里写"判官 +N"，那些数字在 CI 变绿之前都该打折看。
- **验证**：tools_smoke 两代全量+三改动文件单编零真错；第 21 闸自证红/复绿；21 闸全绿（0.1.510 对表）。零配置零协议零存档改动，
  **主线业务方法体零改动**（只加探针助手），1.20.1 只改判官。**实机不需要验，看 CI**：推完轮询三个 job——「Gradle 编译出包」应
  第一次自 m481 以来变绿；「1.20.1 GameTest」九条应转绿（若仍有红，从 `ci-retro-errors` 再读）；「GameTest 用例集」是主线判官
  **自 m481 以来第一次真跑**，很可能冒出一批新失败——那是下一刀（读 `ci-gametest-report`）。
- **下一刀**：读 CI 三分支；主线 GameTest 若红→修；全绿后回 A4。

## m510b 热修（CI 首跑抓获）：1.20.1 最后一条判官——"搬仓"把去处指回了源仓

- **CI 结果（m510 推后 4 分钟）**：「Gradle 编译出包」**自 m481 以来第一次绿**；「GameTest 用例集」（主线判官）**自 m481 以来第一次真跑，
  全绿**——主线判官全部真存在了；「1.20.1 编译+GameTest」九条转绿八条，剩 `extractor_pump_respects_sink_and_whitelist` 一条：
  "搬仓模式该开抽，仓里还剩 100"。
- **根因**：判官给抽取节点补 kind0 产出边时把去处指回了**同一个** src 仓。泵（`SupplyPuller`）与清运（`LogicNodeTicker`）同拍
  （pullSupply 在生产循环之前，清运 5t 同拍）——同一拍里先从 src 抽 100 进缓存、再原样清运回 src，拍末账面恒 100，
  "搬仓"判据永远不成立。不是业务错：搬仓的语义就是"从 A 搬到 B"，B=A 时零净变化是对的。
...
## m511b 记档：m511 已推 main，CI 八 job 全绿

- 对接文档里那枚 PAT 401，m511 先按规矩本地提交 + `format-patch` 交付；作者当轮贴了新 PAT，push 前 fetch 对表 origin/main 仍是
  01c2642（m510b），`b617f28`=m511 推上 main。run 33853443731 **八 job 全绿**（Gradle 编译出包 / GameTest 用例集 / 1.20.1 编译+GameTest /
  26.1 / 26.2 / NeoForge 1.21.1 / 配方校验+资源审计 / 成员对照表代产）——m511 动了两代客户端屏 + 新 xplat 共用件，主线 javac 与
...
## m513 A7a：1.20.1 节点菜单 / 存储连线菜单条目（主线原文只装本世代有的操作）+ connectedComponent 下沉共用 —— 右键一键摘回退役

- **来路**：作业表 A7（设置面板/帮助/节点菜单条目/拾取器）一刀切不完，按"一刀一段"拆：**A7a=菜单条目**（本刀），A7b=整理布局 autoLayout，
  A7c=设置面板 m199 + 帮助卡 m219；拾取器一族（合成目标/药水/附魔/交易/作物/过滤/监测…）背后是 setNodeTarget 与各机制，本世代没有，**不搬**
  （m489 律）。开工先 fetch，origin/main=a54f600（m512 CI 八 job 全绿）。
- **逐句对**：主线 `openNodeMenu`（原 1619~1799）= 标题带机器名 → 暂停/恢复 → 融合拆解 → 断开全部连线 → 二十余条机制条目 → 组合两入口
  （m264）→ 取出机器（危险项垫底）→ 取消；存储端点右键菜单（原 2032~2050）= 标题"存储连线" → 收回总线（m265 放置卡）→ 断开全部连线 → 取消。
  **世代口是机制面**：本世代四操作（放/移/摘/连 m457）+ 存储连线三态循环（m458）+ 分组（m192）——过滤后剩：断开全部连线 / 组合所选 /
  组合相连 / 取出机器 / 取消；存储菜单剩：断开全部连线 / 取消。装配助手 `mi/mt/addMenu×4/clearMenu/openMenu/cmenu.title` m509 已留壳，
  **主线 addMenu 原文逐条粘**（文案/图标/style 全同：`mt("disconnect_all")` 2 组首、`mt("group_selected")` 2、`CHAIN` 组合相连 style 按所选
  数取 0/2、`mt("remove_machine")` 1 危险红显、取消 2）。
- **共用件**：`connectedComponent`（m264 BFS 连通分量，"组合相连"入口）从主线屏下沉 `GroupFrameRenderer.connectedComponent(connections, n, idx)`
  静态（原文逐句，`be.connections()/be.nodes().size()` 改形参），主线留同签名转发壳（1 处调用点零改动）。
- **1.20.1 屏**：`openNodeMenu/openStorageMenu/clearLinksOfMachine/clearLinksOfStorage` 四方法挂在 `sendQuery()` 后；**右键节点由"一键摘回"改为开菜单**
  （主线 m148 口径：取出机器是菜单里垫底红显的危险项——原右键即摘一手滑就丢机器，与主线体验不同）；右键存储卡开"存储连线"菜单（原右键无响应）。
  `clearLinksOfMachine`：机器线逐对 `NodeLink(cut=true)`；存储边走本世代三态循环包——**产出态(0)要转两格才到断、供料态(1)一格**，同一包按态连发
  1~2 次（服务端按序处理、每包各推一次快照，中间那帧短暂显示为供料线；不新造"按向断"包，复用现有循环包，m509 "先复用已有"同律）。
  `clearLinksOfStorage` 同法。取出机器条目的动作照搬原右键那支（发包 + `sendQuery` + `linkFrom == idx` 归零）。
- **资源**：`remove_machine/disconnect_all/group_selected` 三张菜单贴图拷进 1.20.1 资源目录（主线原图字节同），第 19 闸「菜单贴图 mt 逐张对表」
  8→11 句全对上。
- **对照表**：第三节补「存储连线包语义」「节点菜单条目面」两行。
- **验证**：`tools_smoke.py` 两代全量（192/88）零真错 + 三改动文件单编零真错（CanvasScreen120 147 / GroupFrameRenderer 11×2 / 主线屏 557 全噪音）；
  自家 11 个新符号 symbol 级 0；`NodeGroup` 四参/`NodeLink` 四参/`StorageLink` 三参/`NodeRemove` 二参/`connectedComponent` 三参逐处手数
  （记录型构造与两代 payload 记录逐字段核）；21 闸全绿（0.1.513）。**零协议改动**（全走 1.20.1 现有五包）、零存档、零配置；纯客户端。
  判官零新增（client 类不可载）。
- **实机验证脚本**（1.20.1）：①右键一台机器 → 弹主线同款菜单：标题=机器名，「断开全部连线」组首、「取出机器」垫底红字红条、「取消」；
  点「取出机器」机器回背包（改前右键即摘，现在要过菜单）；②机器 A 连 B、A 连存储卡（产出态）→ 右键 A「断开全部连线」→ A-B 线与 A-存储线全断
  （存储线中间可能闪一帧供料色属正常）；③把 A-存储线滚到供料态再断一次 → 一包到位；④Shift 选两台后右键其一 → 「组合所选(N台)」建组；
  A-B-C 串连后右键 A → 「组合相连(3台)」建组；⑤右键存储卡 → 「存储连线」菜单，「断开全部连线」红显，该卡所有边断；⑥菜单开着滚轮/点别处的
  modal 行为同 m509；⑦主线：右键节点/存储端点菜单应与 m512 逐位一致（connectedComponent 转发壳）。
...
## m517 A10：1.20.1 节点卡层进世界矩阵（随 zoom 缩）—— 存储卡线口回 24 屏幕实占（m458 起悬空）

- **来路**：作业表 A10（m512 登记：缩放范围放开到 5%~800% 后卡互叠更刺眼）。开工先 fetch，origin/main=7dac8cc（m516 CI 八 job 全绿）。
- **逐句对**：主线机器层=`translate(panX,panY)+scale(zoom)` 下 `drawNode(ctx, be, i, nx, ny, st)`（世界坐标）+ 升级格/齿轮/暂停覆层（本世代无机制，
  m489 律不装）+ m387 悬停原因浮窗（本世代走 render() 里的 tooltip 整句 m464/m493，等价不搬）→ popPose → 选中高亮/框选（共用件自带同一变换，pop 之后调）。
  本世代 m484 起卡面整张走同一份 `NodeCardRenderer`，**只是调在屏幕坐标层**（`sx/sy` 换算后直画）——同一份代码在两代画出不同尺寸，又是"共用件读到的
  环境两代一样吗"（m511 教训第四例：矩阵/线色/作用域/**坐标层**）。命中框 m507 已改 `NW*zoom`，与本刀之后的画法一致。
- **1.20.1 屏**：节点卡循环挪进上面机器线层 push 的那份世界矩阵（中间无别的绘制，帧序不变），坐标改 `wnx/wny` 世界值直传；视口裁剪换算成屏幕坐标判
  "整卡出屏才裁"（原 `x<-32||…` 在屏幕坐标层的近似裁剪退役）；连线首端高亮描边同进矩阵（世界单位随卡同缩，与主线 m508 选中高亮同律）；
  `popPose` 挪到卡层之后（注释同步）。**顺手**：m484 遗留的死 `ring` 取色 switch（m483 自编灯点退役时忘拔）拔除。
  **存储卡不随 zoom（刻意）**：主线放置卡是总线卡，`bw()/bh()` 只随总线倍率不随 zoom（m265 口径），本世代 24×24 屏幕实占与之同律，卡本身不动；
  但 m458 起线口按 `stW = 24*zoom` 算——卡画的是 24 屏幕像素，线口却按缩放后的卡算，zoom≠1 时产出/供料线口悬在卡外（zoom 2.5 时偏出 36px）；
  两处（单线循环 + m511 `StoragePorts` 口）改回 `24f`，与 `storageAt` 命中框（一直是 24）一致。
- **对照表**：第三节「卡片层与缩放」行改"m517 两代同律"，并记存储卡两代都不随 zoom 的依据。
- **验证**：`tools_smoke.py` 两代全量零真错 + CanvasScreen120 单编零真错（159 条噪音同 m516）；自家 6 符号 0；`drawNode` 七参不变；21 闸全绿（0.1.517）。
  零协议零存档零配置零资源；纯客户端；主线零改动（本刀只动 1.20.1 一文件）。
- **实机验证脚本**（1.20.1）：①放几台机器，滚轮 0.4→2.5：**卡随 zoom 缩放**（改前只挤位置），5% 档卡不再互叠；卡面文字/图标/灯点随缩；
  ②连线首端高亮描边随卡同缩；拖卡/悬停/右键命中框与卡同框（m507 口径）；③机器接存储卡：zoom=1/0.5/2 三档线口都落在存储卡底边 1/4 与 3/4 处
  （改前 zoom≠1 悬空）；存储卡本身尺寸不随 zoom（主线放置卡同律）；④归并线（m511）到存储卡的落点同上；⑤主线无改动。
- **下一刀**：读 m517 CI → **A9 悬停聚焦 m164b**（主线 `hovAny/hovN/hovEnd` 三件 + 两处分流 `lit` + 非归并线的 `mix(termInk, wire, 0.30)` 压暗；
  本世代 m511 起分流处 `lit` 恒真即接口；主线 `hoveredNode()`/悬停端点判定先逐句对）→ A8 升级槽（先核本世代有无升级系统，m464 记档：无）。

## m518 A9：1.20.1 悬停聚焦 m164b（指着哪张卡，只有它的线全亮，其余压暗三成）—— 两处分流 lit 接口兑现

- **来路**：作业表 A9（m511 登记：两处分流传的 `true` 即接口）。开工先读 m517 CI：八 job 全绿；fetch，origin/main=3726bd3。
- **逐句对**：主线 `renderBg` 的 m164b 块=①`hovN/hovEnd` 两件（世界坐标倒序命中节点，命中不到再命中总线放置卡）→ `hovAny`；
  ②存储线循环 `boolean lit = !hovAny || mi == hovN || e[1] == hovEnd`；③机器线循环 `lit2 = !hovAny || c[0] == hovN || c[1] == hovN`；
  ④非归并线颜色式 `lit ? wireOut() : SciSkin.mix(termInk(), wireOut(), 0.30f)`（进线同式 wireIn）；归并线与徽章的 lit 早在 m511 共用件 `WireBundler` 里。
  API 面（作业表第 1 步）：wmx/wmy/wnx/wny/storageAt 全是本世代已有助手，`SciSkin.mix/termInk` 共用件现成，**1.21 专属为零**——预计的"世代差为零"成立。
- **1.20.1 屏**：块整段搬进 `renderBg`（底层之后、分组共享表之前，主线同序），归一后与原文的差**只有存储端那一支**：主线 m265 总线放置卡
  `snx/sny + bw()/bh()`（含 `busVisible()/endPlaced` 收起态过滤）五行 → 本世代 `storageAt(mouseX,mouseY)` 一句（与点击/悬停详情同一口，24×24 屏幕命中）；
  节点命中 `wnx(i)/wny(i)`（拖动幽灵同律）、卡高 `NH`（主线 `NH+26` 含升级格行）。两处分流 `true`→`lit`/`lit2`，三处非归并线颜色式照原文替换，
  `drawWire` 十一参不变。m511 那行"本世代无 m164b 悬停聚焦，lit 恒真"注释改为接线记录。
- **对照表**：第三节「悬停聚焦」行改"m518 起两代同律"，记三处记法差。
- **验证**：`tools_smoke.py` 两代全量零真错 + CanvasScreen120 单编零真错（159 条噪音同 m516/m517）；自家 8 符号（hovAny/hovN/hovEnd/lit2/seH/mix/termInk/storageAt）
  全量与单编两份日志各 0；归一 diff=存储端一支；六式（两 lit + 四色式中的三处非归并线）主线/本世代各 1；21 闸全绿（0.1.518）。
  零协议零存档零配置零资源；纯客户端；主线零改动（本刀只动 1.20.1 一文件 + 两份文档）。
- **实机验证脚本**（1.20.1）：①放三台机器互连+接一张存储卡，鼠标不指任何卡：全部线全亮（与 m517 前逐位一致）；
  ②悬停某台机器：只有它的进/出线（机器线 + 到存储卡的产出/供料线）全亮，其余线压暗成三成底色；移开恢复全亮；
  ③悬停存储卡：只有接它的产出/供料线全亮；④建组后跨组归并线与 ×N 徽章：悬停组成员时归并线与徽章随 lit 同亮/同暗（m511 共用件那半这次才真被喂到 false）；
  ⑤拖动卡时悬停判定跟着幽灵位（wnx/wny 同律）；⑥主线无改动。
...
## m521 作者点名·零代码笔：主线(1.21.1)↔1.20.1 全量缺口普查 + 超大工作台移植分片（docs/主线对比1201_全量缺口与超大工作台普查_m521.md）

- **来路**：作者纠正「我说的是超大工作台」（m520 我理解成了自动合成机——**没问清就动手**，守则第 2 条犯了一次），并问「能不能根据 1.21.1 的东西去对比」。
  对：前几十刀按作业表一件件搬，**从没有一张以主线为基准的整体缺口表**，作业表四条线里根本没登记超大工作台，所以作者看到的"没做"我账上是空白。m520 CI 八 job 全绿后开工。
- **做法**：`git ls-files` 类清单 / 两目录模型逐名 / 包 record 数 / 子系统目录计数，**机械对比不靠记忆**。结果：方块 8→4（缺超大工作台、交易所、卫星+无线节点）；
  屏 7→2（缺五屏）；机器 107 台两代同源 ✅（m453 数据驱动），tick 覆盖生成/耗料/熔炉/六族 ✅，自动合成/酿造/附魔/交易/复制/作物多选/虚空/区块族 ✗；
  非机器物品缺 21（含四种升级包）；升级系统整套 ✗；包 24→13；命令/JEI/mixin 三个子系统 1.20.1 为零；画布 A 线 ✅、存储 B 线 ✅。
  **读法：能玩的主链全通，但玩家"造机器"的入口——超大工作台（引子签名配方）在 1.20.1 不存在，机器只能创造栏拿——这是当前最大的玩法缺口，排 C4 之前。**
- **超大工作台专项量**：`SuperBenchRecipes` 1039 行/1 处 parse、`SuperBenchScreenHandler` 592 行/10 处三类（parse×6、`getComponentsPatch`×3、`CUSTOM_NAME`×1）、
  `CraftGridInventory` 0 触点 → **三文件可整文件上挂 1.20.1 白名单**；`SuperBenchScreen` 392 行只 2 parse + 四参 render 签名 → 绘制体下沉共用件、屏留壳（m515 手法）；
  `SuperBenchBlock` 55 行 `codec()`/`useWithoutItem` → 世代壳（B4b 同款）；注册三件 `MenuType(…, FeatureFlags.VANILLA_SET)`/`SimpleMenuProvider`/`MenuScreens.register` 1.20.1 同签名。
  **一个新口**：现成的 id 解析口 `SciSkin.Gfx.id` 在 client 包，handler 跑服务端不能引——要服务端可用的口（先 grep 现成，无则进 `ItemData.Impl`，顺带收 `resetHoverName`）。
- **分片**：SB2 世代口+三文件上挂（第 20 闸自证）→ SB3 1.20.1 方块壳/BE/注册/资源六件套 → SB4 屏绘制体共用件+两代壳 → SB5 判官（配方唯一/端到端合成/服务端权威）。
  作业表新开 **S 线**排 C4 前；其余缺口按建议序登记（交易所→升级系统→命令→跨维度→随身仓库+mixin→JEI）。
- **验证**：零代码改动；21 闸全绿（0.1.521）。
- **教训**：①「X 没做吗」这类问题的正确第一步是**查一张以主线为基准的缺口表**而不是猜作者指哪件——表现在有了；②作者说"合成"我按作业表里最近的词（自动合成机）
...
## m523 SB2b：超大工作台 handler 四口重做 → `SuperBenchScreenHandler`/`CraftGridInventory` 重挂 1.20.1 白名单（38→40）—— 菜单类型安装口 + 物品宿主口（十口）；第 20 闸加候选模式「先闸后挂」；评审四条进作业表

- **来路**：m522b CI 八 job 全绿（含 1.20.1 编译+GameTest，m510 规矩：读完才开刀）。按作业表取 SB2b。开工前评审（作者转达，m518~m522b 六笔复核）四条意见同刀处理，见末段。
- **先核前提再动手（m521 稿量错两次的教训）**：①handler 全部 MC 触点逐个过 `docs/世代API对照表.md`+yarn 1.20.1 映射：`Slot.setByPlayer`（yarn `setStack` method_48931，1.20.1 在）/`Slot.tryRemove`（`tryTakeStackRange`）/`copyWithCount`/`Player.level()`/`MenuType(MenuSupplier,FeatureFlagSet)`/`clickMenuButton`/`canTakeItemForPickAll`/`clearContainer` 等全部两代同名同签名；
  ②`asCraftInput` 删覆写的前提**查 1.21.1 源码**（GitHub 代码搜索 MCP-1.21 三处）：`CraftingContainer` 接口 `default asCraftInput() = asPositionedCraftInput().input()`、`default asPositionedCraftInput() = CraftingInput.ofPositioned(getWidth(),getHeight(),getItems())`、`CraftingInput.of(w,h,items) = ofPositioned(w,h,items).input()`——与被删覆写体**逐位同义**（原注释写"很可能已有 default"是猜的，这次是核的）。
- **两口落地（handler 592→656 行，四类依赖全部出户）**：
  - **菜单类型安装口** `installType(MenuType<SuperBenchScreenHandler>)`/`reqType()`（ItemData.install 样板：重复装抛、未装用抛），构造器 `super(reqType(), syncId)`；主线 `ModScreenHandlers` 在 `SUPER_BENCH` 字段后 `static { installType(SUPER_BENCH) }`（`SuperBenchScreenHandler::new` 只是方法引用不触发类初始化，无循环）；1.20.1 随 SB3 注册后装。
  - **物品宿主口** `Host` 十个 default 方法（`packsAvailable/isPack/tier1/tier2/packInnerId/packRawCount/packRatio/packOf/cagedType`），`installHost/host()`（未装用抛）。主线 `screen/LegacySuperBenchHost` 十口**原表达式原句照搬**（`instanceof CompressedPackItem`/`ModItems.COMPRESSED_PACK`/`CompressedPackItem.of/innerId/rawCount`/`((CompressedPackItem)s.getItem()).ratio`/`instanceof CaptureCageItem ? cagedType : null`），Sdzjz.onInitialize 首段装；1.20.1 RetroBootstrap 装 `new Host(){}` 空宿主=十口全走默认关。
  - 触点按 m522b HEAD 原文逐处手数（grep 计数）：`CompressedPackItem` 17 处（`rawCount`×1、`innerId`×7、`instanceof`×6、`of`×3）+ `pk.ratio`×3（pattern 变量绑定一次→`int ratio = host().packRatio(s)` 循环前取一次，shrink 到空后 getItem 变 AIR 同律）、`ModItems` 两件包×8、`CaptureCageItem`×8（四处判定各两次，收成 `mob.equals(host().cagedType(s))`，实现里 instanceof 守卫留着——防带 `caged` 键的非笼物品误配）、`ModScreenHandlers`×2（import+super）。改后残留 grep `com.sdzjz.item.Compressed|Capture|com.sdzjz.registry`（剥注释）=0。
  - **默认关时不静默吞件（作业表第 2 条明令逐句核）**：`compressGrid` 若在无包世代跑到 `packOf(null,…)` 会把 64 件散件换成一只不存在的包→新增 `packsUnavailable(player)` 总闸在 compress/unpack 首行，报「此版本没有压缩材料包，压缩区不可用」零动作（服务端权威：钮 id 来自客户端，屏上不画钮也得拦）；`pullPacked` 默认关=t1/t2 为 null、`getItem()!=null` 恒真前两轮扫空只搬散件；`gridMultiset` 认包恒否全按散件计；刷怪类配方 `mobOk` 恒假（1.20.1 无抓物笼，m521 缺口表 21 件之列——**已知缺口**：缺料提示仍会说"去抓笼子"，随抓物笼移植到序）。主线十口同义转发，行为零变化。
- **`CraftGridInventory`**：`asCraftInput` 覆写删除（前提见上），注释改成可核的三句源码引用；剩余 API 面 `SimpleContainer/CraftingContainer/StackedContents.accountSimpleStack` 两代同名。
- **第 20 闸加候选模式（评审第 3 条：SB2 的教训是验收顺序倒置——先上挂、构建红、再补闸）**：`python3 docs/tools_gen_api_check.py --candidate com/sdzjz/screen/X.java …` 把未入白名单的 xplat 文件按"已挂"一起量（两类判据都过，候选彼此可见）。**自证=m522b 血案重放**：m522b HEAD 那两份原文当候选→红 5 处（CraftingInput/ModScreenHandlers/ModItems/CompressedPackItem/CaptureCageItem，与作者构建+第 20 闸当时报的五处逐一对上）→本刀改后复绿→再写 build.gradle→再跑一次绿。**上挂顺序自此固定：候选先过闸→再写白名单→再跑一次**，作业表流程第 6 步前加一步。
- **判官 +1（累计 67）**：主线 `super_bench_host_pack_roundtrip`——mock 玩家开 handler，网格 64 圆石→压缩钮→应恰 1 只装圆石的一级包+0 散件→拆开钮→包清零、圆石守恒 64（连背包一起数）。**它钉的是"宿主口装了没"**：默认关是静默的（钮只报不可用、散件原样躺着），漏装 `installHost` 主线压缩区就悄悄没了；菜单类型安装口由构造器 `reqType()` 顺带判（漏装构造当场抛）。
- **验证**：`tools_smoke.py` 两代全量（主线 197 / retro 93→95 文件）+ 十份单编零真错；自家 17 个新符号 `symbol:` 行 0、`does not override abstract`/`incompatible types` 涉本刀 0；第 20 闸候选模式红 5→绿、正式模式绿；21 闸全绿（0.1.523）。零协议零存档零配置零资源；**主线行为零变化**（十口同义转发 + 安装口只换取处）；1.20.1 仍只是编译进去（无方块/菜单注册，玩家开不到），SB3 接。
- **实机验证脚本**：①主线超大工作台照旧——浏览器填料/放料合成/shift 搬料/**压缩区两钮（64 散件→一级包→拆回）**/刷怪配方要装着生物的笼子/合成后笼子清空留网格；任何差异都是回归。②1.20.1 无可见变化（作者判据：拉取重构建 1.20.1 BUILD SUCCESSFUL；CI 1.20.1 job 绿）。
- **评审四条的处理（作者转达 m518~m522b 复核）**：
  ①缺口定性=功能覆盖非复刻质量、超大工作台排 C4 前——与作业表一致，不动。
  ②**`CanvasScreen120` 992 行是最大隐性负债**（A9/m519 是"搬进世代壳"不是"下沉共用件"，主线画布每改一处要人工同步一刀且无闸管）——量了：renderBg 181 行 + mouseClicked 143 + mouseReleased 111 + mouseDragged 29 ≈ 464 行是主线原文搬入段。**我的取舍（技术实现方式，自己定）：走下沉不走"接受现状"**——真移植路线下"对表闸的萎缩速度=进度条"（m498），给搬入段立对表闸是仿写路线的补丁，与路线相反；登记 **A12 CanvasScreen120 减负**（连线两循环+m164b 悬停聚焦+m193 分流 → xplat `CanvasWireLayer`；m519 拖线三段状态机 → `CanvasWireDrag`，发包留各代；先量 API 面，世代差预期只剩 StoragePorts/包名/三态连线三处已知口）排 SB5 之后；**过渡期把它写进取活检查单**：主线 `StructureCoreScreen` 改动落在搬入段（`grep m164b|m519|m193` 锚点）→同刀同步或登记欠账，不许静默漂。
  ③先闸后挂→候选模式落地（见上）；**SB3~SB5 每刀开工先跑 21 闸再动笔**写进流程。
...
## m526b 热修（m526 CI 1.20.1 判官红）：判官②首次实机对表纠正两笔账——材料白名单两条过期滚掉、1.20.1 配方可达 3→**0**/126；可达账落成 `EXPECTED_REACHABLE` 等值断言，③④账说 0 时明说空转

- **现象**：m526 CI 1.20.1 job 红，`ci-retro-server-tail.log` 三条判官失败：②`缺口白名单过期该滚：[材料白名单 sdzjz:trade_center 不在配方表, 材料白名单 sdzjz:villager_contract 不在配方表]`；③④`无可达配方`。编译绿、其余判官绿、**没有"新缺口"**——10 种 1.21 原版料 + core_module/capture_cage/wireless_node 就是 1.20.1 解析不出的全集。
- **根因（两笔都是离线尺子的错，判官对表干的正是这个）**：①m526 离线脚本把 `addSmall9("sdzjz:trade_center", …)` 的**结果件**归进了材料白名单——它们只在结果位出现；②离线正则只认 `"sdzjz:…"` 字面量，`addSmall9` 那批用字母常量 `MM="sdzjz:core_module"`——三条"可达"（data_panel/storage_core/data_cable）九格里都有 MM。**真账：126 条可达 0**。
  **m109 家法再验一次**：校验器自身先怀疑——离线脚本量出的数没在实机对过表就写进了 DEVLOG/作业表/m521 稿三处，这刀统统改回。
- **修法**：①材料白名单滚掉两条（剩 10 原版 + capture_cage/core_module/wireless_node）；②加 `EXPECTED_REACHABLE = 0` 常量，判官② 从 `reachable > 0` 改**等值断言**（多了少了都红），并断言 `smallestReachable()==null` 与 `EXPECTED==0` 一致；③判官③④选不出配方时 `assertTrue(EXPECTED_REACHABLE == 0)` 再 `succeed`——**空转是登了账的**：SB6 注册 core_module 后 reachable>0 → ②红 → 逼着同刀改常量 → ③④自动转真跑（不是 m257 那种没人知道的假绿）。
- **验证**：retro 冒烟零真错；21 闸全绿；版本仍 0.1.526（热修不抬）。作业表 SB5 行/阻塞区/下一刀、m521 稿状态段三处 3/126 → 0/126 同刀改回。**作者判据：CI 1.20.1 job 转绿（判官②③④三条）。**
...
## m533 评估报告入账（零业务代码）——逐条处置稿 + 版本能力表首版 + 作业表重排（U/P 线新开、N3 拆三、C 线解阻塞、F1-0 ✅/F1e）+ 第 13 闸「xplat→src 依赖闭包」段 + 1.20.1 README/两句运行时文案

- **取活**：作者 c921ff1「新建议」= 根目录《SDZJZ项目评估与开发规划 .md》（外部评估，基线 **m530b/0.1.530**，17 章：现状核查/五问题/阶段路线/F1 拆法/1.20.1 排序/架构边界/完成定义/文档与玩家体验/两周清单/待决项/API 选型与四边界/API-01~07）。作者只说「继续，我上传了一份新建议在仓库里」→ 按协作风格「说了就照做」：**先入账再开刀**，排期类自己定、玩法/文档结构类进阻塞区不猜。m532 CI 三 run 全绿、22 闸绿，开工。
- **报告 vs 现状的差**（读报告先知道）：F1a/F1b 已落（报告写 F1 未开工）；`src/` 待拆 3；报告点名的两条文档冲突属实（1.20.1 README「无玩法」/A8 销项 vs N1 引 A8）。
- **量出的事实（F1-0 依赖闭包，报告 P0-①）**：xplat 显式 import 根 src 类 **23 文件 / 9 类**——`Sdzjz`(2)、四 BE `StructureCore/StorageCore/DataPanel/DataCable`(4/4/4/2)、`TradeCenterBlockEntity`(2)、`ModBlockEntities`(6)、`ModItems`(7)、`ModScreenHandlers`(6)。**8 类已零 Fabric（F1a/F1b 之功），1 类 `StorageCoreBlockEntity` 在待拆表=F1c**。结论修正报告：F1d 本就是 NeoForge **整挂 common+xplat+src（排除胶水）**，xplat→src import 不是阻断；要盯的是「xplat 不许引 src **胶水**」。
- **第 13 闸 `tools_layer_gate.py` 新段 ③**：剥注释后抓 `import com.sdzjz.…;` 逐级缩短匹配 src 顶层类（吃静态导入/内部类）；命中 `SRC_GLUE_OK` → **红**（共用层绑加载器入口）；命中 `SRC_GLUE_PENDING` → **报数**（F1c 收官应归零）。**自证**：临时让 `xplat/node/NodeUpgrades.java` `import com.sdzjz.loader.FabricEntry` → 红点名一处；还原 → 绿「23 文件引 9 个 src 类（零胶水 ✓）；引待拆件 1 类：StorageCoreBlockEntity←4文件」。
- **落地四份文档**：①`docs/评估报告入账_m533.md`——基线差表 / 闭包表 / §三~§十七 逐条处置（采纳·改造·不采纳·已过时各给一句理由）/ 本刀清单；②`docs/版本能力表.md` **首版**——报告 §八 七态口径（**定**义/**注**册/**配**方可解析（只证 id 可解析，判官②口径）/生存可**得**/行为可**运**/保存恢复**存**/**客**户端验收），四格（1.21.1 Fabric / 1.20.1 Fabric / 1.21.1 NeoForge / 26.x）分表，**没证据写「？」不猜**（1.20.1 全部「客」列为「？」=D1 欠验四条从没跑过；主线「客」◐=作者日常在玩无逐项清单；Create 联动两代只有代码无锁定版本组合实测）；末节「这张表还缺什么证据」明列；③`docs/作业表.md`：**U 线新开**（U1 升级安装/数值生效/画布槽（原 A8 `drawUpgradeSlots` 整段搬并入）/Shift 批量；U2 融合阶位+存储升级分别核消费方）、**P 线新开**（P1 能力表维护 / P2 停机原因码（先问作者，碰主线灯表与协议）/ P3 Beta 验收清单（报告 §八 六条证据 + 性能矩阵沿 `/sdzjz bench`）/ K1 Create 互通实测 / 候选不入待办：Jade·EMI·KubeJS·CC:Tweaked·公开 API·联网更新·液体·能量）、**N3 拆 a/b/c**（跨维度绑定族 / 交易族 / 随身族；logo 随资源整理、chunk_data_core 随区块族）、**C 线**：C2b 拆 **brew**（无阻塞）/**ench**（阻塞附魔定价），**C3 依赖「C2」改「—」**（核过 `xpPool` 进项=熔炉族 `0.1*done`（m494 注释）+ 虚空处理器，不依赖酿造附魔；原表笔误），C4a 排 1.20.1 轨道第一刀、C4b 注「先单目标」、C5 扩成特种机型逐族；**F 线**：F1-0 ✅（本刀）、F1c 验收补报告 §14.1 双侧+八场景+结算口径、F1d 开工首步并入 API-01（NeoForge 21.1 坐标/`IItemHandler`+`Capabilities.ItemHandler.BLOCK`+`RegisterCapabilitiesEvent`/`PayloadRegistrar`/`IMenuTypeExtension`/`@GameTestHolder`；`ModBlocks/ModItems` 静态初始化即注册→`RegisterEvent` 内触发）+ 收官判据改「服务器起+客户端进+核心工作台可放+最小生产链」、**F1e 新增**（JEI 入口/mixin 族/判官全跑/CI 升玩法测试）；「下一刀」段加**两条新规矩**（能力表同刀更新；1.20.1 轨道新序 **C4 → U1 → U2 → C3 → C2b-brew → N3-a/b/c → A12 → C5 → 兼容**，F1c/F1d 优先、之后两轨交替）；阻塞区 +3（首批 Beta 范围 / HANDOVER 结构是否瘦身 / 人力工期折算——只记不催）；「其余缺口」行标各项归属；A8 行批注更正；归档 +m533。④`versions/1.20.1/README.md`「骨架+Common 真编译+在岗日志，无玩法」过期段 → 现状摘要（指向能力表）+ 源集/数据包生成规矩。
- **唯一代码改动（1.20.1，非业务）**：`StructureCore120` 两句运行时黄灯文案去开发分片编号（报告 §九「用玩家语言替换运行时出现的开发分片编号」）：L438 `"配方/特种机型随 C2-⑤ 后续分片到序（本世代暂只跑数据驱动生成类）"` → `"当前版本尚未支持该机器（配方/特种机型）"`；L498 `"组件产物机型随 C2-⑤d（精确账本对接）到序"` → `"当前版本尚未支持带附加数据的产物（如山羊角）"`；刀号留注释。灯级 2 不变、分支不变；判官 grep 不断言这两句字面；BMP 字符。
- **不采纳/未动（理由一句）**：HANDOVER 首页瘦身（作者文档，须点头→阻塞区）；10~18 周工期表（作业表不做承诺）；SCBE/画布「拆大类」独立任务（守则 6/8，按 F1 触点抽）；液体/能量/EMI/KubeJS/CC:Tweaked/公开 API/联网更新（无需求）；独立「一页现状」文档（能力表兼任，少一份要同步的东西）。
- **验证**：两代冒烟零真错（main 206 / retro 110 全量 + `StructureCore120` 单编零真错）；第 13 闸红/绿自证；22 闸全绿（0.1.533）。零协议零存档零配置零资源；**主线零改动**；1.20.1 运行时可见变化=两句黄灯文案。
  **作者判据**：1.20.1 放一台特种/配方机型（如复制器）接仓开机→黄灯读「当前版本尚未支持该机器（配方/特种机型）」；山羊角机型→「当前版本尚未支持带附加数据的产物（如山羊角）」。
- **教训**：①外部评估的价值在「它不知道我们已经做了什么」——先做基线差表再逐条处置，否则会把已落地的 F1a/F1b 当待办重做；②「依赖有多少」和「依赖是不是阻断」是两个问题——报告数出 23 个文件是对的，结论「工作量偏乐观」要看 F1d 到底挂不挂 src，量完只需一条闸盯「不许引胶水」；③能力表最难的不是填 ✅ 而是敢填「？」——1.20.1 一整列「客」都是「？」，这比一列 ✅ 更有信息。
- **下一刀**：读 m533 CI → **F1c 存储传输适配器**（`StorageCoreBlockEntity` 的 `Storage<ItemVariant>` 提供侧拆进 `src/loader/FabricStorageAdapter`，FabricEntry 注册；第 13 闸依赖闭包待拆计数 1→0、`SRC_GLUE_PENDING` 3→2）。**开工先跑 22 闸。**

## m534 F1c：存储传输提供侧适配器——`StorageCoreBlockEntity` 内部类 `FabricLedger` 整段搬成 `src/loader/FabricStorageAdapter`，BE 只留不透明缓存槽 `transferAdapter(Supplier)`；`DataCableBlockEntity.coreStorage` 改走既有 `Xfer.find`（零新口）；`src/` 待拆欠账 3→2、依赖闭包引待拆 1→0

- **取活**：m533 CI 全绿、22 闸绿 → 按作业表取 F1c。触点量清（m532 记档 5 符号属实）：`StorageCoreBlockEntity` L4-8 五个 FTA import + 字段 `fabricLedger` + `fabricStorage()` + 内部类 `FabricLedger extends SnapshotParticipant<Integer> implements Storage<ItemVariant>`（66 行，含 `View`）；消费点四处：`FabricEntry` 提供侧注册 lambda、`DataCableBlockEntity.Host.coreStorage`（回收拍目的地）、`SdzjzGameTests` 七处、`FabricXfer` 类注一句。1.20.1 的 `StorageCore120.FabricLedger120` **不动**（世代壳自有 Fabric 1.20.1，F3/F4 再拆）。
- **搬法（作业表第 2 节纪律）**：按签名定位 + 括号配平整段切（不按手打行号，m529b 规矩），66 行原文机械替换**三类**：类名 `FabricLedger`→顶层 `FabricStorageAdapter`（去一级缩进）/ `setChanged()`→`be.setChanged()`（BlockEntity 公开方法）/ `FabricLedger.this.extract`→`FabricStorageAdapter.this.extract`；其余一字未改（m278 增量事务日志注释、m350 撤键拷贝、坏档脏 id 静默跳过、`Long.MAX_VALUE` 容量全在）。脚本三类替换各恰一次断言，写盘前 `setChanged();` 裸调用残留=0。新增：`be`/`ledger` 两字段（`ledger = be.ledger()`，原内部类直摸外层同名字段）、私有构造、`static Storage<ItemVariant> of(be)`。
- **BE 侧唯一新东西=不透明缓存槽**：`private Object transferAdapter; public Object transferAdapter(Supplier<Object> create)`——首次建、之后恒返同一实例。**为什么必须缓存**：`SnapshotParticipant` 的 undo 日志住在适配器实例里，同一 BE 若每次 `find` 都新建实例，嵌套事务的快照位点就串账（原字段 `fabricLedger` 也是为此缓存的，只是类型换成 Object）。业务层不解释这个 Object；NeoForge 的 `IItemHandler` 适配器（F1d）用同一个槽：`event.registerBlockEntity(Capabilities.ItemHandler.BLOCK, STORAGE_CORE_BE, (be, side) -> (IItemHandler) be.transferAdapter(() -> new NeoForgeStorageAdapter(be)))`。**Object 字段合规性**：Phase 1.5 三规②「Object 句柄名册内禁存字段」管的是 Common 名册（`docs/common_manifest.txt`），`src/` BE 不在名册；`common_gate` 绿。
- **自家消费点两处**：①`FabricEntry`：`(be, direction) -> FabricStorageAdapter.of(be)`；②`DataCableBlockEntity.Host.coreStorage(probe)`：原 `((StorageCoreBlockEntity) core).fabricStorage()` 直取=业务 BE 上的加载器符号 → 改 `Xfer.find(be.getLevel(), be.getBlockPos(), null)`（m434 既有口，**零新口**）。Fabric 侧 `ItemStorage.SIDED.find` 按坐标取 BE→提供侧 lambda→`of(be)`→缓存槽 → **与原直取逐位同一个对象**；多一次按坐标查 BE（回收拍频度，可忽略）；NeoForge 侧 F1d 走能力查询。判官七处 `c.fabricStorage()`→`FabricStorageAdapter.of(c)`（该文件在待拆表 F1d，Fabric 符号合法）。
- **顺手**：BE 三个随内部类变死的 import 拔除（`Item`/`ResourceLocation`/`Iterator`——逐个对剥注释后的类体 grep=0 才拔，`BuiltInRegistries/List/ArrayList` 仍有人用留着；m472「死 import 也是世代触点」）；`Xfer.java` 类注「刻意留在原地的两处①」改写为「m533 已兑现」、`FabricXfer` 类注补一句；BE 两处过期注释（fabricStorage 显式注册）改词。
- **第 13 闸账本**：`SRC_GLUE_OK` +`loader/FabricStorageAdapter`（11）；`SRC_GLUE_PENDING` 删 StorageCoreBlockEntity 行 → **剩 2**（F1d `SatelliteNodeModel` ModelLoadingPlugin / `SdzjzGameTests` FabricGameTest+传输断言）；**依赖闭包段：23 文件引 9 个 src 类、引待拆件 0**（m533 立段时预言「F1c 收官应归零」兑现）。**第 19 闸**：BE 已删符号 +`fabricStorage`。
- **验证**：两代冒烟零真错（main 207 / retro 110 全量 + 七改动文件单编零真错）；自家新符号 symbol 级报错 0（`FabricStorageAdapter`/`transferAdapter`/`coreStorage`/`ledger`——精确 grep `symbol: (class|variable|method) X`，不数文件名出现次数）；新文件 symbol 行全是 `ItemVariant/TransactionContext/StorageView/Storage/SnapshotParticipant/ResourceLocation/ItemStack` MC/Fabric 类型 + 一条 `method setChanged()`（`be` 父类 BlockEntity 不可解析的继承噪音，Mojmap 1.21.1 该方法 public）；22 闸全绿（0.1.534）、第 21 闸 253 文件 2727 处裸调用对表绿；括号配平/import 完整性/悬空引用绿。零协议零存档零配置零资源；**主线行为零变化**（Fabric 侧最终仍是同一份 `SnapshotParticipant`+`Storage<ItemVariant>` 实现，只是类名与住址变了，实例仍按 BE 缓存）。
  **7 类盲区自查**：`SnapshotParticipant<Integer>` 两抽象 `createSnapshot/readSnapshot` + `onFinalCommit` 覆写原句 ✓；`Storage<ItemVariant>` 三口 `insert/extract/iterator` 原句 ✓；`StorageView` 五口原句 ✓；`Supplier<Object>` 收 `() -> new FabricStorageAdapter(be)` 协变 ✓；`(Storage<ItemVariant>) Object` 未检查转型有 `@SuppressWarnings("unchecked")` ✓；`Xfer.find(Level, BlockPos, Direction)` 传 `null` 面（门面注释「null=不分面」）✓；私有构造由同类静态 `of` 调 ✓。
  **作者判据 / 实机脚本**：①任意走 fabric-transfer-api 的管道（Create 传送带/漏斗、MI、TechReborn）怼存储核心存取普通件与附魔书/药水精确件——行为与 m533 前逐位一致（存得进、取得出、精确件不混堆不变裸）；②数据线回收模式从 Create 传送带收货入核心（走 `coreStorage`→`Xfer.find` 新路）——货进账本、tag 件入精确账本；③事务回滚：管道模组半路取消的事务，账本原样（判官 `fabric_abort_restores_normal_entry`/`fabric_nested_abort_restores_exact_entry` 在 CI 主线 GameTest 里走新 `of(c)` 路径）；④原版漏斗贴核心底面塞件照旧（幻影槽路径未动）；⑤退出再进账本原样（存档路径未动）。**CI 主线 GameTest job 必须绿**（46 判官中 FTA 相关七处调用点改了）。
- **教训**：①「刻意留在原地」的注释也会过期——m434 写「等目录分层时整体搬进 versions/<loader>」，真到 F 线发现正确住址是 `src/loader/`（与 FabricNet/FabricXfer/FabricMenus 同族），而不是另开目录；旧结论过期时把新结论写回原注释旁，别只在 DEVLOG 记；②「业务 BE 上的加载器类型公开方法」是最容易漏的一类 Fabric 触点——`fabricStorage()` 返回 `Storage<ItemVariant>`，五个 import 都是为它存在；抽掉它之后 BE 剩下的是一个 Object 槽，**槽的存在理由（实例缓存=事务日志归属）要写在槽上**，不然下一个人会把它「优化」成每次新建。
- **下一刀**：读 m534 CI → **F1d NeoForge 模块编译**（作业表 F 线：开工首步 API-01 锁坐标与接口 → build.gradle 挂 `common`+`xplat`+`src`（排除 `SRC_GLUE_OK` 11 文件 + 待拆 2）→ `NeoForgeEntry/NeoForgeClientEntry` + 四口实现 + `Menus` 口 + 注册时机 `RegisterEvent` 内触发 ModBlocks/ModItems/ModBlockEntities/ModScreenHandlers 类初始化 + `SatelliteNodeModel` 模型插件世代口 + 判官注册口；收官判据=服务器起+客户端进+核心工作台可放+最小生产链）。**开工先跑 22 闸。**

## m535 F1d-1：NeoForge 模块整挂三源集 + 入口与五口 NeoForge 实现（Env/Hooks/Menus/Net/Xfer 消费侧）+ 注册分期（Fabric 零行为变化）+ `MenuData.menuCodec` + 第 13 闸两判据 + CI 错误回推

- **取活**：m534 CI 八 job 全绿、22 闸绿 → 按作业表取 F1d，**开工首步 API-01**：对官方 1.21.1 文档逐口核名（registries / networking·payload / gui·menus / concepts·events / inventories·capabilities），落成 `docs/F1d_NeoForge排刀稿_m535.md` 第一节对照表（每行带出处），F1d 拆四刀（本刀=编译目标，F1d-2 客户端、F1d-3 提供侧能力+判官、F1d-4 收官验收）。**沙箱没有 NeoForge jar，冒烟对 `net.neoforged.*` 全盲——CI NeoForge job 是本刀唯一编译判官。**
- **注册分期（评估报告 P0 点名的生命周期检查点，作业表 F1d 行预告）**：量清=全树直接 `Registry.register` 只在四注册类（BLOCK 8 / ITEM 8+N / BLOCK_ENTITY_TYPE 8 / MENU 6 / CREATIVE_MODE_TAB 1），靠类初始化触发；NeoForge 每个注册表只在自己的 `RegisterEvent` 期解冻，`ModBlocks.reg` 一句里方块+方块物品同注册 → BLOCK 期碰 ITEM 表必抛 "Registry is already frozen"。处置（**Fabric 零行为变化**）：①`ModBlocks.reg` 只注册方块，八个 `BlockItem` 挪 `ModBlocks.registerBlockItems()`（包私有静态，按字段顺序），`ModItems` 静态段**首句** `static { ModBlocks.registerBlockItems(); }` → ITEM 注册表内顺序逐位不变（八方块物品仍在一切物品之前；BLOCK 顺序不变；跨注册表先后无人依赖）；②`ModItems.initItems()` 无体触发口（ITEM 期），`init()` 仍=创造栏注册（CREATIVE_MODE_TAB 期）；③`Sdzjz.init()` 拆 `initPorts()`（世代口/配置/命令/压测/服务端 tick 钩，不碰注册表）/ `initRegistries()`（四句）/ `initHooksAndNet()`（平台钩 + 全部 payload），`init()`=三段顺序调用，**语句与顺序逐位不变**，FabricEntry 仍调 `init()`。静态初始化连带核过（`LegacySuperBenchHost` 只在方法体摸 ModItems；`initHooksAndNet` 顶层无注册表触点）。
- **漏网的 Fabric 胶水引用**：`SdzjzClient.init()` 首句 `ClientNet.install(new FabricClientNet())`——类名不含 `net.fabricmc`，第 13 闸旧正则抓不到，NeoForge 整挂 src 时就是 cannot find symbol。挪 `FabricClientEntry`（ClientHooks 安装句之后、`SdzjzClient.init()` 之前，仍早于一切客户端接收器）。**第 13 闸新判据④**：白名单外 src 文件剥注释/字符串后出现任一 `SRC_GLUE_OK` 简单类名 → 红（自证：放回那句当场红点名 `FabricClientNet`，还原绿）。
- **`MenuData<D>` 加第二口 `menuCodec()`**（四 BE + TerminalItem 各一行 `BlockPos.STREAM_CODEC`，与 `ModScreenHandlers` 里 `Menus.type` 传的同一个）：Fabric 的 `ExtendedScreenHandlerType` 自带 codec、开屏时从菜单类型找；NeoForge 的 `openMenu(provider, Consumer<RegistryFriendlyByteBuf>)` **写数据那一步拿不到菜单类型**——m532 写「NeoForge 那边就是 `openMenu(provider, buf -> codec.encode(...))` 一句」时预设了 codec 在手，现在补上由 provider 自报。FabricMenus 不用它（零改动）。
- **NeoForge 模块**（`versions/1.21.1/neoforge/`）：①`build.gradle` 挂 `common`+`xplat`+`src` 三源集，`exclude` 排 11 胶水 + 2 待拆 + `mixin/**`（F1e），资源挂 `src/main/resources` 排 `fabric.mod.json`/`sdzjz.mixins.json`，`-Xmaxerrs 1000`；m408 骨架类注「Yarn↔Mojmap 墙」过期文案改写。**第 13 闸新判据⑤**：按 Ant 通配把 `SRC_GLUE_OK ∪ SRC_GLUE_PENDING` 13 文件逐一对 exclude 表，没盖住 → 红（自证：删一条 exclude 当场红点名，还原绿）——两份手写名单从此不漂。②`SdzjzNeoForge`（m408 骨架，报一行日志）退役 → `NeoForgeEntry`（`@Mod`）：装五口 → `Sdzjz.initPorts()` → 挂 `RegisterEvent`（`getRegistryKey()` 对 `Registries.BLOCK/ITEM/BLOCK_ENTITY_TYPE/MENU/CREATIVE_MODE_TAB` 五期各触发一句）+ `RegisterPayloadHandlersEvent` → `Sdzjz.initHooksAndNet()`。③`NeoForgeEnv`（ModList/FMLPaths）④`NeoForgeHooks`（六口=六事件，全挂游戏总线；EntityInteract 非 PASS → setCancellationResult+setCanceled，m94 抓物笼「抢在 interact 之前」语义同）⑤`NeoForgeMenus`（`IMenuTypeExtension.create((id, inv, buf) -> factory.create(id, inv, codec.decode(buf)))`；open 走 `menuCodec().encode`）⑥`NeoForgeNet`：**两处世代差都在时机**——registrar 出事件即失效而业务在构造器就登记几十个包 → 每笔缓冲成 `Consumer<PayloadRegistrar>` 事件来了逐条落；`playToServer` 要类型+codec+处理器齐全而 Fabric 分两口 → 处理器给**晚绑定分派**（落地只登记转发 lambda，真处理时查 `SERVER` 表，`onServer` 事件前后调都行）；s2c 转 `CLIENT_DISPATCH` 口（F1d-2 客户端入口装，专服恒 null，本类零客户端类引用）；线程默认主线程与 Fabric 一致；`toPlayer`=`PacketDistributor.sendToPlayer`。⑦`NeoForgeXfer` 消费侧五口：`level.getCapability(Capabilities.ItemHandler.BLOCK, pos, side)`；`insert` 按堆叠上限分批 `ItemHandlerHelper.insertItem`（exact=false 裸物品同 Fabric `ItemVariant.of(Item)`）；`move` 逐源槽「模拟抽→模拟塞→按能塞下的量真抽→真塞」，真塞余量原路塞回源槽/源任意槽，仍余=对方容器口是心非记 warn 停手（报告 §14.1「结算以真塞为准、模拟只用于规划、绝不落地不蒸发」）。提供侧 `IItemHandler` 与判官留 F1d-3。
- **CI**：NeoForge job 加 `permissions: contents: write` + 失败错误清单回推 `ci-neoforge-errors` 分支（照主线 m414 原文，`tee gradle-build.log`）；沙箱读法同三分支（raw 域）。
- **验证**：两代冒烟零真错（main 207 / retro 110 全量 + 十一改动文件单编）；NeoForge 胶水六文件与三源集一起 javac 纯语法冒烟：语法类错 0、自家 15 个新符号 symbol 级 0（余 141 条全是 net.neoforged/net.minecraft 缺 jar 噪音）；第 13 闸两新判据红/绿自证；22 闸全绿（0.1.535）。**Fabric 侧零协议零存档零配置零资源、行为零变化**（ITEM 注册表顺序逐位同前；`init()` 三段=原语句原顺序）。
  **7 类盲区自查**：`RegisterEvent#getRegistryKey()` 返 `ResourceKey<? extends Registry<?>>` 与 `Registries.X` equals ✓；`IContainerFactory.create(int, Inventory, RegistryFriendlyByteBuf)` 三参 ✓；`StreamCodec<? super RegistryFriendlyByteBuf, D>` 对 `RegistryFriendlyByteBuf` 实参 encode/decode 通配捕获 ✓；`IEventBus.addListener` 显式 lambda 形参类型 `(ServerTickEvent.Post e) ->` 供泛型推断 ✓；`PayloadRegistrar.playToServer(Type<T>, StreamCodec<? super RegistryFriendlyByteBuf, T>, IPayloadHandler<T>)` 与 `IPayloadHandler.handle(T, IPayloadContext)` ✓；`ItemHandlerHelper.insertItem(IItemHandler, ItemStack, boolean)` 返余量 ✓；`ILevelExtension.getCapability(BlockCapability, BlockPos, C)` 上下文 `@Nullable Direction` 传 null ✓。**核不到的（列待编译验证）**：`RegisterEvent` 是否为并行分发（若并行则五期触发要 enqueue——官方文档只说 RegisterEvent 在模组总线逐注册表触发，按顺序假设写）；`PlayerInteractEvent.EntityInteract#setCancellationResult` 在 21.1 的确切签名；`Player.openMenu(MenuProvider, Consumer<RegistryFriendlyByteBuf>)` 定义在 `IPlayerExtension`、`Player` 上可直接调。**这些 CI 会替我说话。**
  **作者判据**：CI「NeoForge 1.21.1：编译出包」job 绿=本刀完；主线 Gradle/GameTest job 绿=注册分期与三段拆分零回归；实机（Fabric）：创造栏、方块物品、四张带数据的屏、终端远程面板应与 m534 前逐位一致。**NeoForge 实机不在本刀判据**（没有客户端入口，进不了世界；F1d-2）。
- **教训**：①「同一句注册两个注册表」在 Fabric 上是省一行，在 NeoForge 上是启动即炸——注册时机是加载器差里最不显眼的一类，因为它不是 API 名的差，是**生命周期**的差，冒烟/闸都看不见，只有真启动或读文档才知道；②第 13 闸抓 `net.fabricmc` 字面抓了 30 刀，`new FabricClientNet()` 这种「按类名引胶水」一直漏着——闸的判据要跟着「什么叫依赖」的定义走：import 字面只是依赖的一种形状；③m532 把口切成「provider 给数据、发数据归加载器」是对的，但「发」需要 codec 这件事被 Fabric 的 `ExtendedScreenHandlerType` 藏起来了——**设计世代口时要把另一家真的写一遍**（哪怕只是伪码），Fabric 帮你做的事在别家可能要你自己做。
- **下一刀**：读 m535 CI → NeoForge job 红则拉 `ci-neoforge-errors` 逐条修（m535b）；绿则 **F1d-2 客户端入口**（`NeoForgeClientEntry` dist=CLIENT：`NeoForgeClientNet`（装 `NeoForgeNet.installClientDispatch`）/`NeoForgeClientHooks` 四口 / `RegisterMenuScreensEvent` / `EntityRenderersEvent.RegisterRenderers` / `SatelliteNodeModel` 世代口 / 压缩包 `IClientItemExtensions`；`SdzjzClient.init()` 按事件拆段同 m535 手法；开工前逐个对文档核名）。**开工先跑 22 闸。**

## m535b 热修：m535 CI NeoForge job 红两根因——①xplat `compat/jei` 引 `mezz.jei.api`（NeoForge 模块未配 JEI 依赖）→ 排除并登记可选依赖表；②`MenuScreens.register` 在 NeoForge 上 private → `ClientHooks` 第五口 `registerScreen`（Fabric=原句，SdzjzClient 六句改口）

- **读 CI**：m535 八 job 七绿一红；`ci-neoforge-errors` 分支（本刀立的回推机制第一次派上用场）90 条错误只两类：`xplat/compat/jei/SdzjzJeiPlugin|SdzjzJeiTransfer` 72 条 `package mezz.jei.api does not exist`；`src/SdzjzClient.java` 六处 `register(MenuType,ScreenConstructor) has private access in MenuScreens`（重复三遍=18 条）。两条都在 m535 排刀稿预告范围（§14.4 可选依赖 / 客户端 RegisterMenuScreensEvent），只是以为能拖到 F1e/F1d-2，编译判官不同意。
- **①JEI**：整挂 xplat 把 JEI 插件带进去了，而 JEI 的 API 只在根 build.gradle（Fabric）配了坐标。处置=NeoForge `build.gradle` `exclude 'com/sdzjz/compat/jei/**'`（外部零引用，grep 核过），JEI 的 NeoForge 侧 maven 坐标与插件发现方式随 **F1e**；**第 13 闸 ⑤ 判据扩面**：新表 `XPLAT_OPTDEP_PENDING`（目录→接上的刀）——不是加载器符号（①不红）但别家没配依赖就编不过，一并要求被排除表盖住，接上依赖后删行即放行；自证：删 exclude 当场红点名两文件、还原绿（盖住 15 文件 / 8 条 exclude）。
- **②MenuScreens.register 是 private**：原版就是 private（`MenuScreens` 静态段自己调），**Fabric API 用 access widener 放开了**，主线三十多刀一直直调没人知道它本来不能调；NeoForge 没放开，要走 `RegisterMenuScreensEvent`。这是真世代差→收口：`ClientHooks.Impl` 第五口 `<M extends AbstractContainerMenu, U extends Screen & MenuAccess<M>> void registerScreen(MenuType<? extends M>, MenuScreens.ScreenConstructor<M, U>)`（泛型与原版签名逐位一致，六个调用点的方法引用推断不变；`ClientHooks` 不在 1.20.1 白名单、唯一实现者 FabricClientHooks，加口零波及）；`FabricClientHooks.registerScreen`=原句 `MenuScreens.register(type, ctor)`；`SdzjzClient.init()` 六句 `MenuScreens.register(` → `ClientHooks.registerScreen(`，`MenuScreens` import 随之拔除（类体 grep=0 才拔）。**F1d-2 的 `NeoForgeClientHooks` 必须实现它**：缓冲，`RegisterMenuScreensEvent` 里 `event.register(type, ctor)`。
- **验证**：两代冒烟零真错（三改动文件单编）；NeoForge 侧全源集纯语法冒烟语法类 0、`registerScreen` symbol 级 0；22 闸全绿；**版本仍 0.1.535（热修不抬数字段）**。Fabric 侧行为零变化（六句屏注册最终仍是同一个 `MenuScreens.register`，多一层转发）。**作者判据**：CI NeoForge job 绿；主线 job 绿（FabricClientHooks 泛型覆写编得过）；Fabric 实机六张屏照开。
- **教训**：①「主线一直能用」不等于「原版允许」——Fabric API 的 access widener 会把 private 方法悄悄变 public，写在业务里三十多刀没人察觉；对 NeoForge 而言这类东西比 API 改名更隐蔽，因为 IDE 里点进去看到的就是 private，只是 Fabric 环境不报。**换加载器时，所有直调的 MC 静态注册方法都要问一句「它在原版里是什么可见性」**（`MenuScreens.register` private / `BlockEntityRenderers.register` public——后者 F1d-2 也建议走事件但不是编译级问题）；②可选依赖的编译单元是「第三类」符号：不是 MC、不是加载器、是别家 mod 的 API——第 13 闸此前只认前两类，这次补了第三类的登记与盖住判据。
- **下一刀**：读 m535b CI → NeoForge job 绿则 **F1d-2 客户端入口**（`NeoForgeClientEntry` dist=CLIENT：`NeoForgeClientNet` + `NeoForgeClientHooks` **五口**（第五口缓冲进 `RegisterMenuScreensEvent`）/ BER 走 `EntityRenderersEvent.RegisterRenderers` / `SatelliteNodeModel` 世代口 / 压缩包 `IClientItemExtensions`；开工前逐个对文档核名）；仍红则再热修。**开工先跑 22 闸。**

## m536 F1d-2a：NeoForge 客户端入口——`NeoForgeClientEntry`（dist=CLIENT）+ `NeoForgeClientHooks` 六口 + `NeoForgeClientNet`；`ClientHooks` 第六口 `registerBlockEntityRenderer`；`SdzjzClient.init()` 拆四段（Fabric 零行为变化），屏/BER 两段在 NeoForge 事件期被触发

- **取活**：m535b CI 八 job 全绿（**NeoForge job 第一次绿=F1d-1 收官**）→ 按作业表取 F1d-2；量 `SdzjzClient.init()` 77 行：除四句 `BlockEntityRenderers.register` 直调外全走口；`SatelliteNodeModel`（Fabric `ModelLoadingPlugin`，其余是原版 `UnbakedModel`）与压缩包 `BuiltinItemRendererRegistry` 两件要单独对文档核 NeoForge 名（`ModelEvent.ModifyBakingResult` 能不能拿到 textureGetter / `IClientItemExtensions.getCustomRenderer`）→ **拆 2a/2b**，本刀 2a 只做编译判官能验的部分。
- **时序问题与 m535 同一条理**：`initScreens/initRenderers` 的实参 `ModScreenHandlers.X / ModBlockEntities.X` 会触发注册类初始化——NeoForge 客户端入口构造器期注册表冻着，不能在构造器里调；而 `RegisterMenuScreensEvent`/`EntityRenderersEvent.RegisterRenderers` 一定在注册完成后触发。**处置=事件期触发业务段而不是缓冲**：`NeoForgeClientHooks.withScreens(e, SdzjzClient::initScreens)` 把当前事件装进字段→跑业务段→卸；`registerScreen/registerBlockEntityRenderer` 直接落进当前事件，期外调抛（不静默）。与两事件先后无关，不用知道 NeoForge 内部顺序。
- **`ClientHooks` 第六口 `registerBlockEntityRenderer`**（原版两家都 public，但 NeoForge 该在事件期注册；泛型与原版 `BlockEntityRenderers.register` 逐位一致）；Fabric 实现=原句；`SdzjzClient` 四句改口。
- **`SdzjzClient.init()` 拆四段** `initGfx / initScreens / initRenderers / initHooksAndNet`，`init()`=四段顺序调用，**语句顺序逐位不变**，FabricClientEntry 仍调 `init()`。`initHooksAndNet` 构造器期可调：接收器进表、tooltip/tick/世界渲染钩挂游戏总线、`registerKey` 先建 `KeyMapping` 返回句柄再在 `RegisterKeyMappingsEvent` 期登记（Fabric 立即注册，NeoForge 只认事件期）；lambda 体里的 `ModItems`/`ChunkRemoverItem` 引用运行期才求值。
- **NeoForge 侧三文件**：`NeoForgeClientEntry`（`@Mod(value=MODID, dist=Dist.CLIENT)`，只在物理客户端构造：装 ClientHooks/ClientNet、`NeoForgeNet.installClientDispatch(NeoForgeClientNet::dispatch)`、initGfx、三事件监听、initHooksAndNet）；`NeoForgeClientHooks`（六口：ClientTickEvent.Post / ItemTooltipEvent（游戏总线）/ KeyMapping 缓冲+RegisterKeyMappingsEvent / RenderLevelStageEvent AFTER_ENTITIES（PoseStack 来自事件、MultiBufferSource=renderBuffers().bufferSource()、相机位=getCamera().getPosition()——m393 那条"只有实体画完后矩阵与顶点口双双非 null"的对位）/ 屏 / BER）；`NeoForgeClientNet`（`PacketDistributor.sendToServer` / 接收器表 / `dispatch` 主线程直调，NeoForge 处理器默认主线程与 Fabric `client.execute` 同义）。
- **验证**：两代冒烟零真错（main 207 / retro 110 全量 + 改动文件单编）；NeoForge 侧全源集纯语法冒烟语法类 0、自家 12 新符号 symbol 级 0；22 闸全绿（0.1.536）。Fabric 侧零协议零存档零配置零资源、行为零变化（四句 BER 与六句屏最终仍是同一原版方法，多一层转发；四段=原语句原顺序）。
  **7 类盲区自查**：`@Mod` 的 `dist` 属性 21.1 有 ✓（20.6+）；`RegisterMenuScreensEvent.register(MenuType<? extends M>, ScreenConstructor<M,U>)` ✓；`EntityRenderersEvent.RegisterRenderers.registerBlockEntityRenderer(BlockEntityType<? extends T>, BlockEntityRendererProvider<T>)` ✓；`ClientTickEvent.Post` 21.x ✓；`ItemTooltipEvent.getItemStack()/getToolTip()` ✓；`RegisterKeyMappingsEvent.register(KeyMapping)` ✓；`RenderLevelStageEvent.getStage()/getPoseStack()/getCamera()`、`Stage.AFTER_ENTITIES` ✓；`KeyMapping(String, int, String)` 原版三参 ✓。**核不到的**：`ItemTooltipEvent` 所在包（`net.neoforged.neoforge.event.entity.player`）与 21.1 是否仍在游戏总线——CI 说话。
  **作者判据**：CI NeoForge job 绿；主线 job 绿；Fabric 实机六屏四 BER 照旧。**NeoForge 实机（第一次可试）**：客户端能进世界、能开画布/面板/交易所/工作台/抽取口/随身仓库六张屏、存储核心/数据线/无线/卫星 BER 在画、tooltip 水印、R 键手持移除器开设置面板；**卫星节点静态模型会是 JSON 兜底（可能与 BER 双重渲染）、压缩包图标是静态贴图**——2b 修。
- **教训**：「注册对象当实参」这件事在 Fabric 上从来不用想（入口跑时注册早完了），换到有分期的加载器就是启动崩——比 API 改名难查，因为代码一个字没错，只是**被叫早了**；解法不是给每个注册类加懒加载，而是让加载器的事件去触发业务段（m535 服务端/本刀客户端同一个形状）。
- **下一刀**：读 m536 CI → 红修；绿开 **F1d-2b**：①`SatelliteNodeModel` 世代口——先对文档核 `ModelEvent.ModifyBakingResult` 21.1 有没有 `getTextureGetter()`/`getModelBakery()`（有→在事件里对 `ModelResourceLocation`（block/satellite_node 各 blockstate）替换成自烘 `Baked`；没有→走 `ModelEvent.RegisterGeometryLoaders` 自定义 loader，模型 JSON 加 `"loader"` 字段（原版/Fabric 忽略未知字段））；Fabric 壳保留 `register()` 原句；②压缩包 `IClientItemExtensions.getCustomRenderer()` → `BlockEntityWithoutLevelRenderer` 壳一句转调 `CompressedPackIcon`，`RegisterClientExtensionsEvent.registerItem`；③销待拆 `SatelliteNodeModel` 行。**开工先跑 22 闸。**

## m537 F1d-2b：卫星节点模型世代口（`SatelliteNodeModel` 去 Fabric 成纯原版 UnbakedModel + Fabric 壳 `FabricSatelliteModel` + NeoForge `ModelEvent.ModifyBakingResult` 自烘替换）+ 压缩包动态图标 NeoForge 壳（`IClientItemExtensions`）；`src/` 待拆 2→1

- **取活**：m536 CI 八 job 全绿（NeoForge 产物 16 MB=三源集真在）→ 按作业表取 F1d-2b。先看资源：blockstate 单变体 `""`→`sdzjz:block/satellite_node`（cube_all）、物品模型独立 `item/generated`、压缩包模型已是 `builtin/entity`（两家共用）。
- **①卫星节点模型**：`SatelliteNodeModel` 134 行里只有 `register()`（m151 `ModelLoadingPlugin.modifyModelOnLoad`）和它的 import 是 Fabric，其余是原版 `UnbakedModel`+自烘 `BakedQuad`。处置=本体留 `src/client`（去 import、`register()`→加载器无关入口 `loadShell()`：读 geo、按 `BER_TAKEOVER` 烘空壳/全几何、读失败 null），Fabric 壳 `client/FabricSatelliteModel.register()` 原句照搬（resourceId 判定三片段逐字对表）调 `loadShell()`；FabricClientEntry 改调壳。**NeoForge 没有加载期替换口**→在 `ModelEvent.ModifyBakingResult`（烘后）用同一份 `loadShell().bake(null, event.getTextureGetter(), BlockModelRotation.X0_Y0)` 自烘一个，`event.getModels().put(new ModelResourceLocation(sdzjz:satellite_node, ""), baked)` 只替 blockstate 那一键（物品模型不动）；bake 只吃 textureGetter（源码核过 baker/state 形参不碰）。**第 13 闸账本**：`SRC_GLUE_OK` +FabricSatelliteModel（12），`SRC_GLUE_PENDING` 删 SatelliteNodeModel → **剩 1**（F1d-3 判官）；NeoForge exclude 删一行（7 条，盖住 15 文件）。
- **②压缩包动态图标**：Fabric `BuiltinItemRendererRegistry.DynamicItemRenderer` → NeoForge `RegisterClientExtensionsEvent.registerItem(IClientItemExtensions.getCustomRenderer, item)`；壳 `NeoForgeCompressedPackRenderer extends BlockEntityWithoutLevelRenderer` 覆写 `renderByItem`（六参与 Fabric render 逐位同序）一句转调两代共用 `CompressedPackIcon`，扫光实现同 Fabric 壳（ENCHANTMENT_GLINT_OVERRIDE）；父类两构造实参本类不用。两包各一个扩展实例（框物品不同）。
- **验证**：两代冒烟零真错（main 207 / retro 110 + 五改动文件单编）；NeoForge 侧全源集纯语法冒烟语法类 0、自家 6 新符号 symbol 级 0；22 闸全绿（0.1.537）。Fabric 侧零协议零存档零配置零资源、行为零变化（模型拦截同一段代码换了住址；渲染器两句不动）。
  **核不到的（CI 说话）**：`ModelEvent.ModifyBakingResult#getTextureGetter()` 21.1 是否存在（20.4 加的，若无→改走 `RegisterGeometryLoaders`+模型 JSON `loader` 字段）；`RegisterClientExtensionsEvent.registerItem(IClientItemExtensions, ItemLike...)` 形参顺序；`ModelResourceLocation(ResourceLocation, String)` 记录构造。
  **作者判据**：CI NeoForge job 绿、主线 job 绿；Fabric 实机卫星节点与压缩包图标与之前逐位一致；NeoForge 实机：卫星节点方块不再是整块立方（BER 独占渲染，不双重）、压缩包图标是动态叠图带扫光。
...
## m540 作者拍板「默认低分辨率、高清可选」——两档**对调**：高清原图整体进可选资源包 `resourcepacks/hd`，默认目录与 1.20.1 由 `tools_pixelate.py` 生成像素版；第 23 闸改盯默认目录

- **拍板**：作者「可以自己选高清图和低分辩率图，在设置上选，默认低分辩率」。m538 做的是"默认高清 + 可选像素包"，现在翻过来。
- **为什么对调而不是"默认启用像素包"**：Fabric 有 `DEFAULT_ENABLED`，NeoForge 的 `addPackFinders` 只有 `alwaysActive`（强制、玩家不能关）没有"默认开可关"，将来 Forge/1.20.1 壳又各是一套——把**默认贴图本身**换成像素版，"默认低清"就不依赖任何加载器机制，两加载器（含将来的壳）天然一致；高清则是普普通通的可选包（现有第七口 NORMAL / alwaysActive=false 原样用）。「在设置上选」=游戏「选项→资源包」勾「生电终结者 · 高清立绘」。
- **数据流（脚本头注）**：作者高清原图=唯一源，住 `src/main/resources/resourcepacks/hd/assets/sdzjz/textures/{item,block}/`（124 物品 + 6 方块 + 76 mcmeta，15 MB——就是原来 jar 里那批）；`--write` 从它生成默认目录像素版（130 张；hd 里已是像素尺寸的件原样拷）+ **1.20.1 同名件镜像**（190 个文件与主线逐字节同源，m527「1.20.1 资源与主线同源」不破）+ hd 的 pack.mcmeta（pack_format 34）。**作者老工作流不变**：高清 png 照旧丢进 `assets/.../item/`，跑一次 `--write`——脚本先「收编」（默认目录里最小边 > 目标尺寸的 png 连 .mcmeta 搬进 hd）再生成；本刀正是用这条收编路把 130 张搬过去的。
- **第 23 闸改判据**：默认目录出现高清尺寸 png → 红（"高清图归 resourcepacks/hd"）；默认/1.20.1 与「由 hd 重生成」逐字节不一致 → 红；hd/pack.mcmeta 漂 → 红；红时打印 `--write` 命令。CI 步骤文案同改。`resourcepacks/pixel` 目录与旧脚本删除。
- **代码改动**：只有 `SdzjzClient` 注册句包 id `pixel→hd` 与文案；壳/口零改动。README/TEXTURES.md 两处改写工作流。
- **验证**：`--write` 后校验绿；1.20.1 190 文件逐字节同主线；两代冒烟零真错；23 闸全绿（0.1.540）。**默认外观变化=全部机器/节点物品图标与五方块贴图变像素版**（作者拍板的预期效果）；零协议零存档零配置；jar 体积基本不变（高清从默认挪进包）。
  **作者判据**：Fabric 1.21.1 进游戏默认看到 32× 像素图标、16× 方块；「选项→资源包」左侧有「生电终结者 · 高清立绘（原图 128× / 64×）」，启用后回到之前的高清；动画照播。1.20.1 默认也是像素版（无高清包可选，另拍）。
- **教训**：「默认选哪档」看起来是一个布尔，落到多加载器上是三种不同机制；把"默认"做进**资源本身**而不是**加载器开关**，机制差就消失了——和 m537 卫星模型"共用本体、各家写时刻"是同一个思路。
- **下一刀**：读 m540 CI → 绿回 **F1d-3b 判官**。**开工先跑 23 闸。**

## m541 作者点名「1.20.1 不够还原 1.21.1，结构核心里很多功能没做全」→ 结构核心补全第一刀：节点配置操作层两代共用（xplat `node/NodeConfig`）+ 1.20.1 五包/接收器/菜单条目

- **现象（作者当轮点名）**：1.20.1 的结构核心"不对、很多功能没做全"。量下来分两层：①**配置层整层缺失**——1.20.1 只有 NodeAdd/Move/Remove/Link/Group 五包，主线的 NodePause/NodeSwitch/NodeFilter/NodeSensor/NodeTarget 五包及其服务端五方法一个都没有；节点右键菜单只剩「断开/组合/取出」。开关节点、抽取节点启停/换挡、传感器、过滤名单、暂停的**判定与清运** m473~m499 早在共用件里跑，玩家却没有任何入口去设置——「逻辑在跑、人碰不到」；②**机型层**：自动合成/作物农场/抓物笼/砂轮/交易/复制/区块族/信标/虚空/幽匿十几台仍是黄灯"尚未支持"，升级系统与经验池未建（作业表 C4/U1/C3/C5，按 1.20.1 轨道顺序接着排）。本刀先补①——它是②里大半机型（要选目标/名单）的前置，也是"逻辑节点能玩"的最后一块。
- **根因**：m513 装菜单条目时把「拾取器一族本世代无机制不搬」扩大成了「配置一族全不搬」——其实五方法里只有拾取器（客户端选物品的窗）是本世代没有的，暂停/开关/换挡/方向翻转这些**一条菜单一个包就能到**，服务端方法更是零 1.21 专属 API。
- **修法（是移植不是重做，m485 家法）**：
  - 主线 SCBE `togglePause/toggleSwitch/toggleFilterEntry/setSensorConfig/setNodeTarget` **五方法整段搬**成 xplat `node/NodeConfig`（静态五口 + `Host`），只做三类机械替换：①`instanceof AutoCrafterItem/ChunkFilterItem/VoidProcessorItem/CropFarmItem/BrewingTowerItem/EnchantFactoryItem` → `NodeTags.defOf == Machines.X` 引用同一性（m472/m486 同款；CropFarmItem 在主线同时挂 CROP_FARM/MEGA_CROP_FARM 两 def，两者都认）②`setChanged(); syncToClient();` → `Host.changed()`（m506 同律）③`level` → `Host.level()` 不透明句柄（CraftPlanner/EnchantPlanner 本就吃 Object）。**归一 diff 脚本核过：只在原文的行=这三类替换点，只在新文的行=`g = h.graph()` 与宿主口调用。**
  - **世代口=机器专属那部分**：toggleFilterEntry 的区块族/信标七段哨兵（#zy/#zrd/#bfx/#bfl/#zsbd/#zm/#zw/#zs）与 setNodeTarget 的 trade/dup/seal 三校验依赖 `ChunkFilterItem/ChunkRemoverItem/InfiniteBeaconItem/DuplicatorItem/TradePlanner` 静态助手——这些类含 1.21 专属 tooltip 签名上不了 1.20.1 白名单，原文整段挪进主线宿主 `nodeCfgHost`（只把段末 `return;` 改 `return true;`=已处理，`#zrd` 段内两处熔断 `return;` 同改）；1.20.1 默认关（ExtractPort.Host 同一副面孔），C5 各机型到序时接上。
  - 主线 SCBE 五方法退役为同签名转发壳（调用点 Sdzjz 接收器零改动）；3866→3781 行。
  - 1.20.1：`NodePayloads120` +五包（字段/串长 128 上限逐位同主线接收器，串走 `Net120.readBoundedUtf` 有界解码）；`StructureCoreMenu120` +五接收器（主线接收器原文顺序：前验→串长熔断→一行转发→直推快照）；`RetroBootstrap` 注册；`StructureCore120` 宿主（`changed()`=只落盘）+五转发；`CanvasScreen120.openNodeMenu` 装主线原文条目：**暂停/恢复运行、切为开启/关闭（开关）、开始/停止抽取 + 抽取量换挡（抽取）、改为高于/低于阈值才抽 + 清除自动启停（抽取已设监测时）、改为高于/低于阈值放行（传感器）、切为黑/白名单（过滤器）**；`pause_node.png` 菜单图标拷入 1.20.1 资源目录（第 19 闸菜单贴图普查盯）。**不装的（随 m542 拾取器下沉）**：配置过滤物品…/抽取白名单…/自动启停·监测物品…/监测物品…/选择烧什么…/吞噬白名单…/选择合成目标 等要开选物品窗的条目——这半才是真·本世代没有的机制。
  - 白名单 46→47（`node/NodeConfig`，先 `--candidate` 过第 20 闸再写 build.gradle，m523 顺序）。
- **判官 +2（1.20.1 累计 76 / 主线 47）**：①`RetroCanvasTests.canvas_node_config_five_ops_and_payload_bounds`——暂停翻转 / 开关·抽取同口翻转且普通机器不响应 / 名单加移·黑白切换·`#xr` 512→4096·64 封顶·普通机器不响应·**本世代 #zy 哨兵默认关不写 zp** / 传感器三键·阈值钳 1e12·空 id 保留物品·§clear·抽取节点同口·普通机器拒 / 自动合成机 ct·普通机器拒 / 五包往返 + 200 字 entry 解码期拒；②主线 `node_config_legacy_host_special_entries_installed`——**宿主口装了没**（m523 教训②默认关是静默的）：`#zy` 让区块过滤器 Y 挡 +1 且不落 fl、`#bfl` 让信标等级 0→1、通用段与 `#xr`、转发壳 togglePause 照旧。
- **验证**：两代冒烟（tools_smoke 全量 + 10 文件单编）零真错、自家 24 个新符号 symbol 级报错 0、归一 diff 只剩机械替换、第 20 闸候选/正式两绿、23 闸全绿（0.1.541）。**零存档改动**（写的都是节点栈上既有键 np/so/xo/fl/fb/xr/si/sv/sl/ct/cr，两代同名同布局）、零配置、**协议 1.20.1 新增五包**（主线协议零改动）；主线行为零变化（五方法转发同一份原文）。
- **实机脚本（1.20.1）**：①画布右键开关节点→「切为:关闭」→卡面读数变 ○关、下游停产；再点回开 ②抽取节点→「开始抽取」→仓→抽取节点→耗料机链真抽，「抽取量: 512/轮 → 换挡」连点五次回 512 ③传感器→「改为:高于阈值放行」文案翻转（监测物品还没入口，等 m542）④过滤器→「切为黑名单」卡面名单标记翻转 ⑤任意机器→「暂停节点」压暗+黄灯停产，「恢复运行」回来 ⑥退出重进世界以上状态原样（键随存档）⑦主线：区块过滤器/信标菜单换挡照旧、开关/抽取/传感器/过滤菜单照旧（同一份代码转发）。
- **教训**：「本世代无机制不搬」这句判断要**按方法拆开量**，不能按功能族一刀切——五方法里四个半零世代差，只有"选物品的窗"一件真缺，却让整层配置在 1.20.1 缺席了三十多刀；作者看到的"很多功能没做全"，有一半是**入口没装**而不是逻辑没搬。
- **下一刀**：读 m541 CI → **m542 拾取器（选择器）下沉共用**（主线 openPicker 一族：搜索框/网格/多模式点选，先量 API 面）→ m543 经验池 C3 → m544 抓物笼/作物农场/自动合成 tick 分支 → U1 升级。**开工先跑 23 闸。**
