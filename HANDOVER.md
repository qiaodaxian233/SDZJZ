...
3. **队列已拍板（用户"按顺序来"）**：G组杂项(m135✅) → 凋灵机(m137✅) → 幽匿线(m138✅) → 砂轮祛魔(m139✅)——**队列四项全部收官**。
   ...
   **m143 机器合并**：用户拍板"整合到一起"——凋灵线5台→wither_farm一台、G组3台→g_misc_machine、
   ...
   创造栏只见3台）；③游戏内验证 m136-m141；④验 m144（砂轮新图标/合同悬停四步用法+职业折扣动态段）；⑤验 m145 村民打折机（吃网络金苹果自动升共网交易所合同折扣，六条验证见 DEVLOG）；⑥待办池其余。
...
4. 真美术：待画仅 villager_discount_machine（m145 新机，现为规整占位）。规格照旧：单机身透明底立绘（元素建议：金苹果+绿宝石+村民剪影），同名覆盖即换。
