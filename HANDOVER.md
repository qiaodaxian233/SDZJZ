...
0. **外部审计②余账对表（m345 登记；P0 CraftPlanner=m343 已销、P1 玩家扫描=m344 已销，均逐行对源核实后才动的手）**：
   - **属实待做**：①SmeltPlanner 同输入多配方 putIfAbsent 只留第一条且序随 RecipeManager 遍历漂
     （核实属实；注意它对 Ingredient 已收全部 matching，与 CraftPlanner 病灶不同层——最小刀=候选列表+
     minecraft 排前字典序稳定，m234 同款，是否要"按库存挑输出"待作者拍板）；②`!running` 前
     tick 头仍跑维护段（快照/端点扫描/区块票等——m344 后无观众快照已零成本，端点扫描 40t/区块票
     20t 本就低频，剩余收益=分级 tick，中优先级）；③CoreChunkLoading restoreClaims 孤儿 claim
     渐进 reconciliation（核实=开服重发票不验核心还在，边缘稳定性）；④DataPanel 网络操作共用
     StorageNetworkSnapshot（deposit/withdraw 各自 cores() 重复解析）；⑤BrewPlanner 全图一次 BFS
