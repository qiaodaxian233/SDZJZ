...
1. 验证 m112（修 m111 整页清空回归）：复现视频操作——拿着东西 shift 点背包物品入库，展示区不再
   全黑；取整组/半组后格子即时显示余量；shift 入库/取出各连打十几下看有无错乱；其余 m111 按键表照验
   （空手左键整组/右键半组、拿着左键全存/右键存1、Shift+右键批量浮层）。m110 地图/暂停顺带。
