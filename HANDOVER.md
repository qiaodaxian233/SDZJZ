...
## 当前状态（m216：存储终端搜索框自绘默认提示（原版placeholder聚焦即隐弃用）；m215：画布上下chrome紧凑化——底带78→56/总线行距收紧/卡尺寸滑块下限0.55并落盘，canvasCompactChrome 开关可回旧版，v15；m214 画布/终端主题分家（画布默认暗夜、终端默认紫晶，v14迁移）；m213 JEI整组填料均分；m212~m215 待编译验证，JEI依赖=全量fabric jar（m209））
