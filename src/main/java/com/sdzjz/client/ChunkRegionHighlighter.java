 ...
 *
 * <p>drawBox=WorldRenderer.method_22980（1.21.1 尚未搬 VertexRendering，yarn 编译级）；分块格线用
 * 退化盒（x1==x2 塌成矩形框）省手搓顶点；透视线走 RenderLayer.getDebugLineStrip（原版 F3+G 区块
...
public final class ChunkRegionHighlighter {
    ...
    private static void drawRegion(MatrixStack ms, VertexConsumerProvider consumers, SdzjzConfig cfg,
        ...
        WorldRenderer.drawBox(ms, vc, x1, y1, z1, x2, y2, z2, cr, cg, cb, pulse);
        WorldRenderer.drawBox(ms, vc, x1 + 0.06, y1, z1 + 0.06, x2 - 0.06, y2, z2 - 0.06,
        ...
        if (r > 0) { // 内部分块格线（退化盒=恒淡竖直隔板框，玩家一眼读出 3×3/5×5）
            for (int gi = 1; gi < w; gi++) {
                ...
                WorldRenderer.drawBox(ms, vc, gx, y1, z1, gx, y2, z2, cr * 0.85f, cg * 0.85f, cb * 0.90f, 0.25f);
                WorldRenderer.drawBox(ms, vc, x1, y1, gz, x2, y2, gz, cr * 0.85f, cg * 0.85f, cb * 0.90f, 0.25f);
            }
        }
    ...
    private static void xray(MatrixStack ms, VertexConsumerProvider consumers,
        ...
        if (consumers instanceof VertexConsumerProvider.Immediate imm) imm.draw(layer);
    }
    ...
    private static void rescan(MinecraftClient mc, String dim, SdzjzConfig cfg) {
        WORKING.clear();
        ...
        net.minecraft.world.chunk.ChunkManager cm = mc.world.getChunkManager();
        for (int dx = -rad; dx <= rad; dx++) {
            for (int dz = -rad; dz <= rad; dz++) {
                int cx = pcx + dx, cz = pcz + dz;
                if (!cm.isChunkLoaded(cx, cz)) continue;
                net.minecraft.world.chunk.WorldChunk ch = cm.getWorldChunk(cx, cz);
                if (ch == null) continue;
                for (net.minecraft.block.entity.BlockEntity be : ch.getBlockEntities().values()) {
