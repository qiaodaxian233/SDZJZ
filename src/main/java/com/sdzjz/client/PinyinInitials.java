...
/** m282 拼音首字母（存储终端搜索"zs"→钻石；零外部依赖，纯客户端）。
 *  一级字库（3755 常用字，GB2312 按拼音排序）走经典区位边界法：字符转 GBK 双字节取无符号码位，
 *  落在 23 段边界哪段就是哪个声母首字母——多音字取字库排序音（行业通行口径，检索足够）。
 *  二级字库（3008 字，按部首排序边界法失效——"燧石/鹦鹉螺壳"实测漏字）走硬表：
 *  pypinyin 离线全量生成、docs/tools_pinyin_check.py 回归尺护表（字符集=GB2312 二级全集逐位核对）。
 *  ASCII 走"词首字母"（Iron Ingot→ii）；其余字符（GBK 扩展罕见字/符号）不产键，静默跳过。
 *  运行时无 GBK 字符集（极端精简 JRE）= of() 恒返回空串，首字母通道静默降级，子串搜索不受影响。 */
public final class PinyinInitials {

    private PinyinInitials() {}

    private static final java.nio.charset.Charset GBK = pickGbk();

    private static java.nio.charset.Charset pickGbk() {
        try { return java.nio.charset.Charset.forName("GBK"); } catch (Exception e) { return null; }
    }

    // 一级字库 23 段起始码位（经典表；docs/tools_pinyin_check.py 逐值断言防手滑）
    private static final int[] B1 = {0xB0A1, 0xB0C5, 0xB2C1, 0xB4EE, 0xB6EA, 0xB7A2, 0xB8C1, 0xB9FE, 0xBBF7,
            0xBFA6, 0xC0AC, 0xC2E8, 0xC4C3, 0xC5B6, 0xC5BE, 0xC6DA, 0xC8BB, 0xC8F6, 0xCBFA, 0xCDDA, 0xCEF4,
            0xD1B9, 0xD4D1};
    private static final char[] L1 = "abcdefghjklmnopqrstwxyz".toCharArray();

    // 二级字库硬表：两串等长按码位序对齐（D8A1..F7FE 共 3008 字）
    private static final String T2_CHARS =
    ...
    private static final String T2_INITS =
    ...
    private static java.util.HashMap<Character, Character> t2;

    private static synchronized java.util.HashMap<Character, Character> t2() {
        if (t2 == null) {
            java.util.HashMap<Character, Character> m = new java.util.HashMap<>(4096);
            for (int i = 0; i < T2_CHARS.length(); i++) m.put(T2_CHARS.charAt(i), T2_INITS.charAt(i));
            t2 = m;
        }
        return t2;
    }

    /** 显示名 → 首字母检索键（全小写）。中文逐字取声母首字母，ASCII 取词首字母，其余忽略。 */
    public static String of(String name) {
        if (GBK == null || name == null || name.isEmpty()) return "";
        StringBuilder sb = new StringBuilder(8);
        boolean prevAsciiLetter = false;
        for (int i = 0; i < name.length(); i++) {
            char c = name.charAt(i);
            if (c < 128) {
                boolean letter = (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z');
                if (letter && !prevAsciiLetter) sb.append(Character.toLowerCase(c));
                prevAsciiLetter = letter;
                continue;
            }
            prevAsciiLetter = false;
            Character hit = t2().get(c); // 二级硬表优先（一级边界法对二级字必然误判/漏判）
            if (hit != null) { sb.append(hit.charValue()); continue; }
            byte[] b = String.valueOf(c).getBytes(GBK);
            if (b.length != 2) continue;
            int v = ((b[0] & 0xFF) << 8) | (b[1] & 0xFF);
            if (v < B1[0] || v > 0xD7F9) continue; // 一级字库区间之外（GBK 扩展等）不产键
            for (int k = B1.length - 1; k >= 0; k--) {
                if (v >= B1[k]) { sb.append(L1[k]); break; }
            }
        }
        return sb.toString();
    }

    /** 查询串是否适用首字母通道（纯 ASCII 字母才开，防中文/数字查询误入）。 */
    public static boolean applicable(String q) {
