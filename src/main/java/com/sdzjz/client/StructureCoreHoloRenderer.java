 ...
 * m250 返工：EntityTranslucentEmissive（method_42600 已核）半透明自发光 + 羽化渐变贴图 +
 * 顶点 alpha 脉动 0.3~0.8——细亮芯柔边光带，不再是平涂胶带。
 */
public class StructureCoreHoloRenderer implements BlockEntityRenderer<StructureCoreBlockEntity> {
    ...
    public void render(StructureCoreBlockEntity be, float tickDelta, MatrixStack matrices,
        ...
        float y0 = 0.10f + tri * 0.76f, y1 = y0 + 0.06f;              // 扫描带 0.06 高（羽化贴图自带柔边）
        float lo = -0.012f, hi = 1.012f;
        int col = argb(0.55f + 0.25f * (float) Math.sin(Math.PI * 2 * phase)); // alpha 脉动 0.3~0.8
        ...
        quad(vc, pm, overlay, col, lo, y0, lo,  hi, y0, lo,  hi, y1, lo,  lo, y1, lo,  0, 0, -1); // 北
        quad(vc, pm, overlay, col, hi, y0, hi,  lo, y0, hi,  lo, y1, hi,  hi, y1, hi,  0, 0, 1);  // 南
        quad(vc, pm, overlay, col, lo, y0, hi,  lo, y0, lo,  lo, y1, lo,  lo, y1, hi,  -1, 0, 0); // 西
        quad(vc, pm, overlay, col, hi, y0, lo,  hi, y0, hi,  hi, y1, hi,  hi, y1, lo,  1, 0, 0);  // 东
    }
