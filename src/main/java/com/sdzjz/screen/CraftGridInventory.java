...
public class CraftGridInventory extends net.minecraft.inventory.SimpleInventory
        implements net.minecraft.inventory.RecipeInputInventory {
    ...
    @Override public void provideRecipeInputs(net.minecraft.recipe.RecipeMatcher finder) {
        for (net.minecraft.item.ItemStack st : this.getHeldStacks()) finder.addUnenchantedInput(st);
    }

    /** 接口很可能已有同签名 default（1.21 便捷法），显式覆写两头保险；getHeldStacks 由 SimpleInventory 协变满足。 */
    @Override public net.minecraft.recipe.input.CraftingRecipeInput createRecipeInput() {
        return net.minecraft.recipe.input.CraftingRecipeInput.create(3, 3, this.getHeldStacks());
    }
}
